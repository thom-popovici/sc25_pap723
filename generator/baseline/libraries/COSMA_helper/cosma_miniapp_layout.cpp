...
bool run(const int m, const int n, const int k, 
         ...
         double& timing, const bool test_correctness,
	 int px, int py,
  ...
  if (!test_correctness) {
    ...
    //CosmaMatrix<T> B(ctx,'B', strategy, rank);
    //CosmaMatrix<T> C(ctx,'C', strategy, rank);
    ...
    //-----------------------------------------------------
    // create a user-defined layout for A
       
    int blockA_m = m / px;
    int blockA_k = k / py;
    std::vector<T> local_dataA (blockA_m*blockA_k);
    fill_int(local_dataA.data(), local_dataA.size());

    auto A_layout = costa::block_cyclic_layout(
					       m, k, // global matrix dimension
					       blockA_m, blockA_k, // block sizes
					       1, 1, // submatrix start 
					       // (1-based, because of scalapack.
					       // Since we take full matrix, it's 1,1)
					       m, k, // submatrix size (since full matrix, it's mat dims)
					       px, py, // processor grid
					       'R', // processor grid ordering row-major
					       0, 0, // coords or ranks oweing the first row (0-based)
					       &local_dataA[0], // local data of full matrix
					       blockA_k, // local leading dimension
					       'R',
					       rank // current rank
					       );
    ...
    //-----------------------------------------------------
    // create a user-defined layout for B
       
    int blockB_k = k / px;
    int blockB_n = n / py;
    std::vector<T> local_dataB (blockB_k*blockB_n);
    fill_int(local_dataB.data(), local_dataB.size());

    auto B_layout = costa::block_cyclic_layout(
					       k, n, // global matrix dimension
					       blockB_k, blockB_n, // block sizes
					       1, 1, // submatrix start 
					       // (1-based, because of scalapack.
					       // Since we take full matrix, it's 1,1)
					       k, n, // submatrix size (since full matrix, it's mat dims)
					       px, py, // processor grid
					       'R', // processor grid ordering row-major
					       0, 0, // coords or ranks oweing the first row (0-based)
					       &local_dataB[0], // local data of full matrix
					       blockB_n, // local leading dimension
					       'R',
					       rank // current rank
					       );
    ...
    //-----------------------------------------------------
    // create a user-defined layout for C
       
    int blockC_m = m / px;
    int blockC_n = n / py;
    std::vector<T> local_dataC (blockC_m*blockC_n);
    fill_int(local_dataC.data(), local_dataC.size());

    auto C_layout = costa::block_cyclic_layout(
					       m, n, // global matrix dimension
					       blockC_m, blockB_n, // block sizes
					       1, 1, // submatrix start 
					       // (1-based, because of scalapack.
					       // Since we take full matrix, it's 1,1)
					       ...
					       'R', // processor grid ordering row-major
					       0, 0, // coords or ranks oweing the first row (0-based)
					       &local_dataC[0], // local data of full matrix
					       blockB_n, // local leading dimension
					       'R',
					       rank // current rank
					       );
    ...
    char transa = 'N';
    char transb = 'N';
    multiply_using_layout(A_layout,
			  B_layout,
			  C_layout,
			  alpha, beta,transa,transb,comm);
    MPI_Barrier(comm);
    double t1 = MPI_Wtime();
...
int main(int argc, char **argv) {
  ...
  auto px = result["procx"].as<int>();
  auto py = result["procy"].as<int>();
  ...
  for (int i = 0; i < n_rep; ++i) {
	...
	result_correct = 
	  run<double>(m, n, k, steps, 
		      t_run, test_correctness, px, py, MPI_COMM_WORLD);
      } else if (type == "float") {
	result_correct = 
