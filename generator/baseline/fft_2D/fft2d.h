...
void benchmark_fft(std::array<int, 2> size_fft, std::deque<std::string> const &args, MPI_Comm comm)
{
    int me, nprocs;
    ...
    if (me == 0)
    {
        cout << size_fft[0] << "x" << size_fft[1] << "\t" << nprocs << "\tFFT direct [s]:\t" << t_direct_max / ntest << "\tFFT inverse [s]:\t" << t_inverse_max / ntest << endl;
    }
}

template <typename backend_tag>
bool perform_benchmark(std::array<int, 2> size_fft, std::deque<std::string> const &args, MPI_Comm comm)
{
...
int main(int argc, char *argv[])
{
    ...
    std::string bench_executable = "./fft2d.x";

    if (argc < 3)
    {
        if (mpi::world_rank(0))
        {
            cout << "\nUsage:\n    mpirun -np x " << bench_executable << " <size-x> <size-y> <args>\n\n"
                 << "    options\n"
                 ...
                 << "Examples:\n"
                 << "    mpirun -np  4 " << bench_executable << " 128 128 -no-reorder\n"
                 << "    mpirun -np  8 " << bench_executable << " 256 256\n"
                 << "    mpirun -np 12 " << bench_executable << " 512 512 -p2p\n\n";
        }
