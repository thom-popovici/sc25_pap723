...
void benchmark_fft(int batch, int batch_proc, std::array<int, 3> size_fft, std::deque<std::string> const &args, MPI_Comm comm)
{
    int me, nprocs;
    ...
    for (int i = 0; i < (batch / batch_proc) * fft.size_inbox(); ++i)
    {
    ...
    {
        if(options.use_pencils == true) 
            cout << size_fft[0] << "x" << size_fft[1] << "x" << (batch / batch_proc) << "\t" << proc_o[0] << "x" << proc_o[1] << "x" << batch_proc << "\tFFT direct [s]:\t" << t_direct_max / ntest << "\tFFT inverse [s]:\t" << t_inverse_max / ntest << endl;
        else
...
bool perform_benchmark(int batch, int batch_proc, std::array<int, 3> size_fft, std::deque<std::string> const &args, MPI_Comm comm)
{
...
int main(int argc, char *argv[])
{
    ...
    if (argc < 6)
    {
        if (mpi::world_rank(0))
        {
            cout << "\nUsage:\n    mpirun -np x " << bench_executable << " <size-x> <size-y> <size-z> <batch> <batch_procs> <args>\n\n"
                 << "    options\n"
    ...
    int batch_dim = std::stoi(argv[4]);
    int batch_proc = std::stoi(argv[5]);
    ...
    init_tracing(bench_executable + "_cuda_double_" + std::string(argv[1]) + "_" + std::string(argv[2]) + "_" + std::string(argv[2]));
