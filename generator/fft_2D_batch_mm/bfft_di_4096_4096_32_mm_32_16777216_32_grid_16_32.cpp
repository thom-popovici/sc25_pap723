...
void function(int cold_runs, int hot_runs)
{
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 256, 4096, 1, 1048576, B0, 1048576, temp0);
		// add measuring things
		...
		unpack_data(256, 256, 16, 1048576, (double*) temp1, 1048576, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[3]);
		fft_compute(CUFFT_FORWARD, fft_plan1, 1, 4096, 256, 1048576, temp0, 1048576, temp1);
		// add measuring things
		cudaEventRecord(counters[4]);
		fft_compute(CUFFT_INVERSE, fft_plan2, 1, 4096, 256, 1048576, temp1, 1048576, temp0);
		// add measuring things
		cudaEventRecord(counters[5]);
		pack_data(256, 16, 256, 1048576, (double*) temp0, 1048576, (double*) temp1);
		// add measuring things
		...
		fft_compute(CUFFT_INVERSE, fft_plan3, 256, 4096, 1, 1048576, temp0, 1048576, temp1);
		// add measuring things
		cudaEventRecord(counters[8]);
		MPI_Alltoall((double*) temp1, 1048576/16, MPI_DOUBLE_COMPLEX, (double*) temp0, 1048576/16,  MPI_DOUBLE_COMPLEX, comm0);
		// add measuring things
		cudaEventRecord(counters[9]);
		unpack_data(256, 256, 16, 1048576, (double*) temp0, 1048576, (double*) Tx0);
		// add measuring things
		cudaEventRecord(counters[10]);
		gemm_compute(gemm_plan, 32, 1048576, 1, 33554432, A0, 1048576, Tx0, 32, temp0);
		// add measuring things
	...
	if(rank == 0)
		printf("32 16777216\tDI\t16 32\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
