...
void function(int cold_runs, int hot_runs)
{
	...
	inx[0] = inembed[0] = onembed[0] = 4096;
	cufftPlanMany(&fft_plan2, 1, inx, inembed, 1,  4096, onembed, 1,  4096, CUFFT_Z2Z, 8192);

	cufftHandle fft_plan3;
	inx[0] = inembed[0] = onembed[0] = 4096;
	cufftPlanMany(&fft_plan3, 1, inx, inembed, 1,  4096, onembed, 1,  4096, CUFFT_Z2Z, 8192);
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 2048, 4096, 4, 33554432, B0, 33554432, temp0);
		// add measuring things
		cudaEventRecord(counters[1]);
		pack_data(4194304, 2, 4, 33554432, (double*) temp0, 33554432, (double*) temp1);
		// add measuring things
		...
		unpack_data(2048, 8192, 2, 33554432, (double*) temp0, 33554432, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[4]);
		fft_compute(CUFFT_FORWARD, fft_plan1, 1, 4096, 8192, 33554432, temp1, 33554432, temp0);
		// add measuring things
		cudaEventRecord(counters[5]);
		fft_compute(CUFFT_INVERSE, fft_plan2, 1, 4096, 8192, 33554432, temp0, 33554432, temp1);
		// add measuring things
		cudaEventRecord(counters[6]);
		pack_data(2048, 2, 8192, 33554432, (double*) temp1, 33554432, (double*) temp0);
		// add measuring things
		...
		unpack_data(4194304, 4, 2, 33554432, (double*) temp1, 33554432, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[9]);
		fft_compute(CUFFT_INVERSE, fft_plan3, 2048, 4096, 4, 33554432, temp0, 33554432, temp1);
		// add measuring things
		...
		unpack_data(2048, 8192, 2, 33554432, (double*) temp1, 33554432, (double*) Tx0);
		// add measuring things
		cudaEventRecord(counters[13]);
		gemm_compute(gemm_plan, 32, 8388608, 4, 268435456, A0, 33554432, Tx0, 128, temp0);
		// add measuring things
		cudaEventRecord(counters[14]);
		MPI_Allreduce((double*) temp0, (double*) C0, 128, MPI_DOUBLE_COMPLEX, MPI_SUM, comm0);
		// add measuring things
	...
	if(rank == 0)
		printf("32 16777216\tDI\t2 8\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
