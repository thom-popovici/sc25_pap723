...
void function(int cold_runs, int hot_runs)
{
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 512, 4096, 1, 2097152, B0, 2097152, temp0);
		// add measuring things
		...
		fft_compute(CUFFT_INVERSE, fft_plan3, 512, 4096, 1, 2097152, temp0, 2097152, temp1);
		// add measuring things
		cudaEventRecord(counters[8]);
		MPI_Alltoall((double*) temp1, 2097152/8, MPI_DOUBLE_COMPLEX, (double*) temp0, 2097152/8,  MPI_DOUBLE_COMPLEX, comm0);
		// add measuring things
		cudaEventRecord(counters[9]);
		unpack_data(512, 512, 8, 2097152, (double*) temp0, 2097152, (double*) Tx0);
		// add measuring things
		cudaEventRecord(counters[10]);
		gemm_compute(gemm_plan, 32, 2097152, 1, 67108864, A0, 2097152, Tx0, 32, temp0);
		// add measuring things
	...
	if(rank == 0)
		printf("32 16777216\tDI\t8 32\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
