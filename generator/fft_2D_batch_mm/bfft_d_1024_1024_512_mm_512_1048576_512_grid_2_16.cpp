...
void function(int cold_runs, int hot_runs)
{
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 512, 1024, 32, 16777216, B0, 16777216, temp0);
		// add measuring things
		cudaEventRecord(counters[1]);
		pack_data(262144, 2, 32, 16777216, (double*) temp0, 16777216, (double*) temp1);
		// add measuring things
		...
		unpack_data(512, 16384, 2, 16777216, (double*) temp0, 16777216, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[4]);
		fft_compute(CUFFT_FORWARD, fft_plan1, 1, 1024, 16384, 16777216, temp1, 16777216, Tx0);
		// add measuring things
	...
	if(rank == 0)
		printf("512 1048576\tD\t2 16\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
