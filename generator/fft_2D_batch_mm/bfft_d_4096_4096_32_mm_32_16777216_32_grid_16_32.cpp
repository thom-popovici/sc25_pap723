...
void function(int cold_runs, int hot_runs)
{
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 1, 4096, 256, 1048576, B0, 1048576, temp0);
		// add measuring things
		...
		fft_compute(CUFFT_FORWARD, fft_plan1, 256, 4096, 1, 1048576, temp0, 1048576, temp1);
		// add measuring things
	...
	if(rank == 0)
		printf("32 16777216\tD\t16 32\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
