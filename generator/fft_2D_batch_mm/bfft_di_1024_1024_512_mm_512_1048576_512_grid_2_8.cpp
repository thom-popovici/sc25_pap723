...
void function(int cold_runs, int hot_runs)
{
	...
	cufftPlanMany(&fft_plan0, 1, inx, inembed, 1,  1024, onembed, 1,  1024, CUFFT_Z2Z, 32768);

	cufftHandle fft_plan1;
	inx[0] = inembed[0] = onembed[0] = 1024;
	cufftPlanMany(&fft_plan1, 1, inx, inembed, 1,  1024, onembed, 1,  1024, CUFFT_Z2Z, 32768);

	cufftHandle fft_plan2;
	inx[0] = inembed[0] = onembed[0] = 1024;
	cufftPlanMany(&fft_plan2, 1, inx, inembed, 1,  1024, onembed, 1,  1024, CUFFT_Z2Z, 32768);

	cufftHandle fft_plan3;
	inx[0] = inembed[0] = onembed[0] = 1024;
	cufftPlanMany(&fft_plan3, 1, inx, inembed, 1,  1024, onembed, 1,  1024, CUFFT_Z2Z, 32768);
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 512, 1024, 64, 33554432, B0, 33554432, temp0);
		// add measuring things
		cudaEventRecord(counters[1]);
		pack_data(262144, 2, 64, 33554432, (double*) temp0, 33554432, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[2]);
		MPI_Alltoall((double*) temp1, 33554432/2, MPI_DOUBLE_COMPLEX, (double*) temp0, 33554432/2,  MPI_DOUBLE_COMPLEX, comm0);
		// add measuring things
		cudaEventRecord(counters[3]);
		unpack_data(512, 32768, 2, 33554432, (double*) temp0, 33554432, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[4]);
		fft_compute(CUFFT_FORWARD, fft_plan1, 1, 1024, 32768, 33554432, temp1, 33554432, temp0);
		// add measuring things
		cudaEventRecord(counters[5]);
		fft_compute(CUFFT_INVERSE, fft_plan2, 1, 1024, 32768, 33554432, temp0, 33554432, temp1);
		// add measuring things
		cudaEventRecord(counters[6]);
		pack_data(512, 2, 32768, 33554432, (double*) temp1, 33554432, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[7]);
		MPI_Alltoall((double*) temp0, 33554432/2, MPI_DOUBLE_COMPLEX, (double*) temp1, 33554432/2,  MPI_DOUBLE_COMPLEX, comm0);
		// add measuring things
		cudaEventRecord(counters[8]);
		unpack_data(262144, 64, 2, 33554432, (double*) temp1, 33554432, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[9]);
		fft_compute(CUFFT_INVERSE, fft_plan3, 512, 1024, 64, 33554432, temp0, 33554432, temp1);
		// add measuring things
		cudaEventRecord(counters[10]);
		pack_data(262144, 2, 64, 33554432, (double*) temp1, 33554432, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[11]);
		MPI_Alltoall((double*) temp0, 33554432/2, MPI_DOUBLE_COMPLEX, (double*) temp1, 33554432/2,  MPI_DOUBLE_COMPLEX, comm0);
		// add measuring things
		cudaEventRecord(counters[12]);
		unpack_data(512, 32768, 2, 33554432, (double*) temp1, 33554432, (double*) Tx0);
		// add measuring things
		cudaEventRecord(counters[13]);
		gemm_compute(gemm_plan, 512, 524288, 64, 268435456, A0, 33554432, Tx0, 32768, temp0);
		// add measuring things
		cudaEventRecord(counters[14]);
		MPI_Allreduce((double*) temp0, (double*) C0, 32768, MPI_DOUBLE_COMPLEX, MPI_SUM, comm0);
		// add measuring things
	...
	if(rank == 0)
		printf("512 1048576\tDI\t2 8\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
