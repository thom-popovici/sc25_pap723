...
void function(int cold_runs, int hot_runs)
{
	...
	B0_host = (std::complex<double>*) malloc(1048576 * sizeof(std::complex<double>));	
	...
	cudaMalloc((void**)&B0, 1048576 * sizeof(std::complex<double>));	
	...
	cudaMalloc((void**)&aux, 1048576 * sizeof(std::complex<double>));	
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 512, 1024, 2, 1048576, B0, 1048576, temp0);
		// add measuring things
		cudaEventRecord(counters[1]);
		pack_data(262144, 2, 2, 1048576, (double*) temp0, 1048576, (double*) temp1);
		// add measuring things
		...
		unpack_data(512, 1024, 2, 1048576, (double*) temp0, 1048576, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[4]);
		fft_compute(CUFFT_FORWARD, fft_plan1, 1, 1024, 1024, 1048576, temp1, 1048576, temp0);
		// add measuring things
		cudaEventRecord(counters[5]);
		fft_compute(CUFFT_INVERSE, fft_plan2, 1, 1024, 1024, 1048576, temp0, 1048576, temp1);
		// add measuring things
		cudaEventRecord(counters[6]);
		pack_data(512, 2, 1024, 1048576, (double*) temp1, 1048576, (double*) temp0);
		// add measuring things
		...
		unpack_data(262144, 2, 2, 1048576, (double*) temp1, 1048576, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[9]);
		fft_compute(CUFFT_INVERSE, fft_plan3, 512, 1024, 2, 1048576, temp0, 1048576, temp1);
		// add measuring things
		cudaEventRecord(counters[10]);
		pack_data(262144, 2, 2, 1048576, (double*) temp1, 1048576, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[11]);
		MPI_Alltoall((double*) temp0, 1048576/2, MPI_DOUBLE_COMPLEX, (double*) temp1, 1048576/2,  MPI_DOUBLE_COMPLEX, comm0);
		// add measuring things
		cudaEventRecord(counters[12]);
		unpack_data(512, 1024, 2, 1048576, (double*) temp1, 1048576, (double*) Tx0);
		// add measuring things
		cudaEventRecord(counters[13]);
		gemm_compute(gemm_plan, 512, 524288, 2, 268435456, A0, 1048576, Tx0, 1024, temp0);
		// add measuring things
		cudaEventRecord(counters[14]);
		MPI_Allreduce((double*) temp0, (double*) C0, 1024, MPI_DOUBLE_COMPLEX, MPI_SUM, comm0);
		// add measuring things
	...
	if(rank == 0)
		printf("512 1048576\tDI\t2 256\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
