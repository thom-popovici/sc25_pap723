...
void function(int cold_runs, int hot_runs)
{
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 1, 4096, 2048, 8388608, B0, 8388608, temp0);
		// add measuring things
		...
		fft_compute(CUFFT_FORWARD, fft_plan1, 2048, 4096, 1, 8388608, temp0, 8388608, temp1);
		// add measuring things
		cudaEventRecord(counters[4]);
		fft_compute(CUFFT_INVERSE, fft_plan2, 2048, 4096, 1, 8388608, temp1, 8388608, temp0);
		// add measuring things
		...
		fft_compute(CUFFT_INVERSE, fft_plan3, 1, 4096, 2048, 8388608, temp0, 8388608, Tx0);
		// add measuring things
		cudaEventRecord(counters[8]);
		gemm_compute(gemm_plan, 32, 8388608, 1, 268435456, A0, 8388608, Tx0, 32, temp0);
		// add measuring things
	...
	if(rank == 0)
		printf("32 16777216\tDI\t2 32\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
