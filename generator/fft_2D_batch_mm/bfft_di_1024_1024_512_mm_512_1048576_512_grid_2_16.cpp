...
void function(int cold_runs, int hot_runs)
{
	...
	B0_host = (std::complex<double>*) malloc(16777216 * sizeof(std::complex<double>));	
	...
	cufftPlanMany(&fft_plan0, 1, inx, inembed, 1,  1024, onembed, 1,  1024, CUFFT_Z2Z, 16384);

	cufftHandle fft_plan1;
	inx[0] = inembed[0] = onembed[0] = 1024;
	cufftPlanMany(&fft_plan1, 1, inx, inembed, 1,  1024, onembed, 1,  1024, CUFFT_Z2Z, 16384);

	cufftHandle fft_plan2;
	inx[0] = inembed[0] = onembed[0] = 1024;
	cufftPlanMany(&fft_plan2, 1, inx, inembed, 1,  1024, onembed, 1,  1024, CUFFT_Z2Z, 16384);

	cufftHandle fft_plan3;
	inx[0] = inembed[0] = onembed[0] = 1024;
	cufftPlanMany(&fft_plan3, 1, inx, inembed, 1,  1024, onembed, 1,  1024, CUFFT_Z2Z, 16384);
	...
	cudaMalloc((void**)&B0, 16777216 * sizeof(std::complex<double>));	
	...
	cudaMalloc((void**)&aux, 16777216 * sizeof(std::complex<double>));	
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 1, 1024, 16384, 16777216, B0, 16777216, temp0);
		// add measuring things
		cudaEventRecord(counters[1]);
		pack_data(512, 2, 16384, 16777216, (double*) temp0, 16777216, (double*) temp1);
		// add measuring things
		...
		unpack_data(262144, 32, 2, 16777216, (double*) temp0, 16777216, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[4]);
		fft_compute(CUFFT_FORWARD, fft_plan1, 512, 1024, 32, 16777216, temp1, 16777216, temp0);
		// add measuring things
		cudaEventRecord(counters[5]);
		fft_compute(CUFFT_INVERSE, fft_plan2, 512, 1024, 32, 16777216, temp0, 16777216, temp1);
		// add measuring things
		cudaEventRecord(counters[6]);
		pack_data(262144, 2, 32, 16777216, (double*) temp1, 16777216, (double*) temp0);
		// add measuring things
		...
		unpack_data(512, 16384, 2, 16777216, (double*) temp1, 16777216, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[9]);
		fft_compute(CUFFT_INVERSE, fft_plan3, 1, 1024, 16384, 16777216, temp0, 16777216, Tx0);
		// add measuring things
		cudaEventRecord(counters[10]);
		gemm_compute(gemm_plan, 512, 524288, 32, 268435456, A0, 16777216, Tx0, 16384, temp0);
		// add measuring things
		cudaEventRecord(counters[11]);
		MPI_Allreduce((double*) temp0, (double*) C0, 16384, MPI_DOUBLE_COMPLEX, MPI_SUM, comm0);
		// add measuring things
	...
	if(rank == 0)
		printf("512 1048576\tDI\t2 16\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
