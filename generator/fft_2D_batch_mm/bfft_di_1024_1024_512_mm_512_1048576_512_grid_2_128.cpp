...
void function(int cold_runs, int hot_runs)
{
	...
	B0_host = (std::complex<double>*) malloc(2097152 * sizeof(std::complex<double>));	
	...
	cufftPlanMany(&fft_plan0, 1, inx, inembed, 1,  1024, onembed, 1,  1024, CUFFT_Z2Z, 2048);

	cufftHandle fft_plan1;
	inx[0] = inembed[0] = onembed[0] = 1024;
	cufftPlanMany(&fft_plan1, 1, inx, inembed, 1,  1024, onembed, 1,  1024, CUFFT_Z2Z, 2048);

	cufftHandle fft_plan2;
	inx[0] = inembed[0] = onembed[0] = 1024;
	cufftPlanMany(&fft_plan2, 1, inx, inembed, 1,  1024, onembed, 1,  1024, CUFFT_Z2Z, 2048);

	cufftHandle fft_plan3;
	inx[0] = inembed[0] = onembed[0] = 1024;
	cufftPlanMany(&fft_plan3, 1, inx, inembed, 1,  1024, onembed, 1,  1024, CUFFT_Z2Z, 2048);
	...
	cudaMalloc((void**)&B0, 2097152 * sizeof(std::complex<double>));	
	cudaMalloc((void**)&C0, 262144 * sizeof(std::complex<double>));	
	cudaMalloc((void**)&temp, 4194304 * sizeof(std::complex<double>));	
	std::complex<double> *temp0 = (temp + 0 * 2097152);
	std::complex<double> *temp1 = (temp + 1 * 2097152);
	cudaMalloc((void**)&aux, 2097152 * sizeof(std::complex<double>));	
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 1, 1024, 2048, 2097152, B0, 2097152, temp0);
		// add measuring things
		cudaEventRecord(counters[1]);
		pack_data(512, 2, 2048, 2097152, (double*) temp0, 2097152, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[2]);
		MPI_Alltoall((double*) temp1, 2097152/2, MPI_DOUBLE_COMPLEX, (double*) temp0, 2097152/2,  MPI_DOUBLE_COMPLEX, comm0);
		// add measuring things
		cudaEventRecord(counters[3]);
		unpack_data(262144, 4, 2, 2097152, (double*) temp0, 2097152, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[4]);
		fft_compute(CUFFT_FORWARD, fft_plan1, 512, 1024, 4, 2097152, temp1, 2097152, temp0);
		// add measuring things
		cudaEventRecord(counters[5]);
		fft_compute(CUFFT_INVERSE, fft_plan2, 512, 1024, 4, 2097152, temp0, 2097152, temp1);
		// add measuring things
		cudaEventRecord(counters[6]);
		pack_data(262144, 2, 4, 2097152, (double*) temp1, 2097152, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[7]);
		MPI_Alltoall((double*) temp0, 2097152/2, MPI_DOUBLE_COMPLEX, (double*) temp1, 2097152/2,  MPI_DOUBLE_COMPLEX, comm0);
		// add measuring things
		cudaEventRecord(counters[8]);
		unpack_data(512, 2048, 2, 2097152, (double*) temp1, 2097152, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[9]);
		fft_compute(CUFFT_INVERSE, fft_plan3, 1, 1024, 2048, 2097152, temp0, 2097152, Tx0);
		// add measuring things
		cudaEventRecord(counters[10]);
		gemm_compute(gemm_plan, 512, 524288, 4, 268435456, A0, 2097152, Tx0, 2048, temp0);
		// add measuring things
		cudaEventRecord(counters[11]);
		MPI_Allreduce((double*) temp0, (double*) C0, 2048, MPI_DOUBLE_COMPLEX, MPI_SUM, comm0);
		// add measuring things
	...
	if(rank == 0)
		printf("512 1048576\tDI\t2 128\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
