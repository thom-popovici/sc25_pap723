...
void function(int cold_runs, int hot_runs)
{
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 512, 1024, 4, 2097152, B0, 2097152, temp0);
		// add measuring things
		cudaEventRecord(counters[1]);
		pack_data(262144, 2, 4, 2097152, (double*) temp0, 2097152, (double*) temp1);
		// add measuring things
		...
		unpack_data(512, 2048, 2, 2097152, (double*) temp0, 2097152, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[4]);
		fft_compute(CUFFT_FORWARD, fft_plan1, 1, 1024, 2048, 2097152, temp1, 2097152, Tx0);
		// add measuring things
	...
	if(rank == 0)
		printf("512 1048576\tD\t2 128\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
