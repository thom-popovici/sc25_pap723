...
void function(int cold_runs, int hot_runs)
{
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 1, 1024, 4096, 4194304, B0, 4194304, temp0);
		// add measuring things
		cudaEventRecord(counters[1]);
		pack_data(512, 2, 4096, 4194304, (double*) temp0, 4194304, (double*) temp1);
		// add measuring things
		...
		unpack_data(262144, 8, 2, 4194304, (double*) temp0, 4194304, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[4]);
		fft_compute(CUFFT_FORWARD, fft_plan1, 512, 1024, 8, 4194304, temp1, 4194304, temp0);
		// add measuring things
		...
		unpack_data(512, 4096, 2, 4194304, (double*) temp0, 4194304, (double*) Tx0);
		// add measuring things
	...
	if(rank == 0)
		printf("512 1048576\tD\t2 64\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
