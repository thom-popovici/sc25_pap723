...
void function(int cold_runs, int hot_runs)
{
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 2048, 4096, 2, 16777216, B0, 16777216, temp0);
		// add measuring things
		...
		fft_compute(CUFFT_INVERSE, fft_plan3, 2048, 4096, 2, 16777216, temp0, 16777216, temp1);
		// add measuring things
		cudaEventRecord(counters[10]);
		pack_data(4194304, 2, 2, 16777216, (double*) temp1, 16777216, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[11]);
		MPI_Alltoall((double*) temp0, 16777216/2, MPI_DOUBLE_COMPLEX, (double*) temp1, 16777216/2,  MPI_DOUBLE_COMPLEX, comm0);
		// add measuring things
		cudaEventRecord(counters[12]);
		unpack_data(2048, 4096, 2, 16777216, (double*) temp1, 16777216, (double*) Tx0);
		// add measuring things
		cudaEventRecord(counters[13]);
		gemm_compute(gemm_plan, 32, 8388608, 2, 268435456, A0, 16777216, Tx0, 64, temp0);
		// add measuring things
		cudaEventRecord(counters[14]);
		MPI_Allreduce((double*) temp0, (double*) C0, 64, MPI_DOUBLE_COMPLEX, MPI_SUM, comm0);
		// add measuring things
	...
	if(rank == 0)
		printf("32 16777216\tDI\t2 16\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
