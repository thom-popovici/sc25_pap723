...
void function(int cold_runs, int hot_runs)
{
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_INVERSE, fft_plan2, 1024, 4096, 1, 4194304, temp1, 4194304, temp0);
		// add measuring things
		...
		fft_compute(CUFFT_INVERSE, fft_plan3, 1, 4096, 1024, 4194304, temp0, 4194304, Tx0);
		// add measuring things
	...
	if(rank == 0)
		printf("32 16777216\tDI\t4 32\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
