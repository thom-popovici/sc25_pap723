...
void function(int cold_runs, int hot_runs)
{
	...
	inx[0] = inembed[0] = onembed[0] = 4096;
	cufftPlanMany(&fft_plan2, 1, inx, inembed, 1,  4096, onembed, 1,  4096, CUFFT_Z2Z, 1024);

	cufftHandle fft_plan3;
	inx[0] = inembed[0] = onembed[0] = 4096;
	cufftPlanMany(&fft_plan3, 1, inx, inembed, 1,  4096, onembed, 1,  4096, CUFFT_Z2Z, 1024);
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 1024, 4096, 1, 4194304, A0, 4194304, temp0);
		// add measuring things
		cudaEventRecord(counters[1]);
		MPI_Alltoall((double*) temp0, 4194304/4, MPI_DOUBLE_COMPLEX, (double*) temp1, 4194304/4,  MPI_DOUBLE_COMPLEX, comm0);
		// add measuring things
		cudaEventRecord(counters[2]);
		unpack_data(1024, 1024, 4, 4194304, (double*) temp1, 4194304, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[3]);
		fft_compute(CUFFT_FORWARD, fft_plan1, 1, 4096, 1024, 4194304, temp0, 4194304, temp1);
		// add measuring things
		cudaEventRecord(counters[4]);
		fft_compute(CUFFT_INVERSE, fft_plan2, 1, 4096, 1024, 4194304, temp1, 4194304, temp0);
		// add measuring things
		...
		fft_compute(CUFFT_INVERSE, fft_plan3, 1024, 4096, 1, 4194304, temp0, 4194304, C0);
		// add measuring things
	...
	if(rank == 0)
		printf("4096 4096 32\tDI\t4 32\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
