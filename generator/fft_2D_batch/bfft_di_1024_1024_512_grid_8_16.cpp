...
void function(int cold_runs, int hot_runs)
{
	...
	A0_host = (std::complex<double>*) malloc(4194304 * sizeof(std::complex<double>));	
	C0_host = (std::complex<double>*) malloc(4194304 * sizeof(std::complex<double>));	
	...
	cufftPlanMany(&fft_plan0, 1, inx, inembed, 1,  1024, onembed, 1,  1024, CUFFT_Z2Z, 4096);

	cufftHandle fft_plan1;
	inx[0] = inembed[0] = onembed[0] = 1024;
	cufftPlanMany(&fft_plan1, 1, inx, inembed, 1,  1024, onembed, 1,  1024, CUFFT_Z2Z, 4096);

	cufftHandle fft_plan2;
	inx[0] = inembed[0] = onembed[0] = 1024;
	cufftPlanMany(&fft_plan2, 1, inx, inembed, 1,  1024, onembed, 1,  1024, CUFFT_Z2Z, 4096);

	cufftHandle fft_plan3;
	inx[0] = inembed[0] = onembed[0] = 1024;
	cufftPlanMany(&fft_plan3, 1, inx, inembed, 1,  1024, onembed, 1,  1024, CUFFT_Z2Z, 4096);

	// create device arrays
	std::complex<double> *A0 = NULL, *C0 = NULL, *temp = NULL;
	cudaMalloc((void**)&A0, 4194304 * sizeof(std::complex<double>));	
	cudaMalloc((void**)&C0, 4194304 * sizeof(std::complex<double>));	
	...
	cudaMemcpy(A0, A0_host, 4194304 * sizeof(std::complex<double>), cudaMemcpyHostToDevice);	
	cudaMemcpy(C0, C0_host, 4194304 * sizeof(std::complex<double>), cudaMemcpyHostToDevice);	
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 1, 1024, 4096, 4194304, A0, 4194304, temp0);
		// add measuring things
		cudaEventRecord(counters[1]);
		pack_data(128, 8, 4096, 4194304, (double*) temp0, 4194304, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[2]);
		MPI_Alltoall((double*) temp1, 4194304/8, MPI_DOUBLE_COMPLEX, (double*) temp0, 4194304/8,  MPI_DOUBLE_COMPLEX, comm0);
		// add measuring things
		cudaEventRecord(counters[3]);
		unpack_data(16384, 32, 8, 4194304, (double*) temp0, 4194304, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[4]);
		fft_compute(CUFFT_FORWARD, fft_plan1, 128, 1024, 32, 4194304, temp1, 4194304, temp0);
		// add measuring things
		cudaEventRecord(counters[5]);
		fft_compute(CUFFT_INVERSE, fft_plan2, 128, 1024, 32, 4194304, temp0, 4194304, temp1);
		// add measuring things
		cudaEventRecord(counters[6]);
		pack_data(16384, 8, 32, 4194304, (double*) temp1, 4194304, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[7]);
		MPI_Alltoall((double*) temp0, 4194304/8, MPI_DOUBLE_COMPLEX, (double*) temp1, 4194304/8,  MPI_DOUBLE_COMPLEX, comm0);
		// add measuring things
		cudaEventRecord(counters[8]);
		unpack_data(128, 4096, 8, 4194304, (double*) temp1, 4194304, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[9]);
		fft_compute(CUFFT_INVERSE, fft_plan3, 1, 1024, 4096, 4194304, temp0, 4194304, C0);
		// add measuring things
	...
	cudaMemcpy(C0_host, C0, 4194304 * sizeof(std::complex<double>), cudaMemcpyDeviceToHost);	
	...
	if(rank == 0)
		printf("1024 1024 512\tDI\t8 16\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
