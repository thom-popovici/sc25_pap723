...
void function(int cold_runs, int hot_runs)
{
	...
	cufftPlanMany(&fft_plan0, 1, inx, inembed, 1,  4096, onembed, 1,  4096, CUFFT_Z2Z, 512);

	cufftHandle fft_plan1;
	inx[0] = inembed[0] = onembed[0] = 4096;
	cufftPlanMany(&fft_plan1, 1, inx, inembed, 1,  4096, onembed, 1,  4096, CUFFT_Z2Z, 512);

	cufftHandle fft_plan2;
	inx[0] = inembed[0] = onembed[0] = 4096;
	cufftPlanMany(&fft_plan2, 1, inx, inembed, 1,  4096, onembed, 1,  4096, CUFFT_Z2Z, 512);

	cufftHandle fft_plan3;
	inx[0] = inembed[0] = onembed[0] = 4096;
	cufftPlanMany(&fft_plan3, 1, inx, inembed, 1,  4096, onembed, 1,  4096, CUFFT_Z2Z, 512);
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 512, 4096, 1, 2097152, A0, 2097152, temp0);
		// add measuring things
		cudaEventRecord(counters[1]);
		MPI_Alltoall((double*) temp0, 2097152/8, MPI_DOUBLE_COMPLEX, (double*) temp1, 2097152/8,  MPI_DOUBLE_COMPLEX, comm0);
		// add measuring things
		cudaEventRecord(counters[2]);
		unpack_data(512, 512, 8, 2097152, (double*) temp1, 2097152, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[3]);
		fft_compute(CUFFT_FORWARD, fft_plan1, 1, 4096, 512, 2097152, temp0, 2097152, temp1);
		// add measuring things
		cudaEventRecord(counters[4]);
		fft_compute(CUFFT_INVERSE, fft_plan2, 1, 4096, 512, 2097152, temp1, 2097152, temp0);
		// add measuring things
		cudaEventRecord(counters[5]);
		pack_data(512, 8, 512, 2097152, (double*) temp0, 2097152, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[6]);
		MPI_Alltoall((double*) temp1, 2097152/8, MPI_DOUBLE_COMPLEX, (double*) temp0, 2097152/8,  MPI_DOUBLE_COMPLEX, comm0);
		// add measuring things
		cudaEventRecord(counters[7]);
		fft_compute(CUFFT_INVERSE, fft_plan3, 512, 4096, 1, 2097152, temp0, 2097152, C0);
		// add measuring things
	...
	if(rank == 0)
		printf("4096 4096 32\tDI\t8 32\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
