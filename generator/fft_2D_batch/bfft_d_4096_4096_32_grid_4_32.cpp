...
void function(int cold_runs, int hot_runs)
{
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 128, 4096, 8, 4194304, A0, 4194304, temp0);
		// add measuring things
		cudaEventRecord(counters[1]);
		pack_data(16384, 32, 8, 4194304, (double*) temp0, 4194304, (double*) temp1);
		// add measuring things
		...
		unpack_data(128, 1024, 32, 4194304, (double*) temp0, 4194304, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[4]);
		fft_compute(CUFFT_FORWARD, fft_plan1, 1, 4096, 1024, 4194304, temp1, 4194304, B0);
		// add measuring things
	...
	if(rank == 0)
		printf("4096 4096 32\tD\t4 32\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
