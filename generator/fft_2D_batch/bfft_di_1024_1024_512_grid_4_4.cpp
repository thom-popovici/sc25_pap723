...
void function(int cold_runs, int hot_runs)
{
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 256, 1024, 128, 33554432, A0, 33554432, temp0);
		// add measuring things
		cudaEventRecord(counters[1]);
		pack_data(65536, 4, 128, 33554432, (double*) temp0, 33554432, (double*) temp1);
		// add measuring things
		...
		unpack_data(256, 32768, 4, 33554432, (double*) temp0, 33554432, (double*) temp1);
		// add measuring things
		...
		unpack_data(65536, 128, 4, 33554432, (double*) temp1, 33554432, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[9]);
		fft_compute(CUFFT_INVERSE, fft_plan3, 256, 1024, 128, 33554432, temp0, 33554432, C0);
		// add measuring things
	...
	if(rank == 0)
		printf("1024 1024 512\tDI\t4 4\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
