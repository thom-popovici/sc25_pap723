...
void function(int cold_runs, int hot_runs)
{
	...
	cudaMemcpy(B0, B0_host, 1048576 * sizeof(std::complex<double>), cudaMemcpyHostToDevice);	
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan1, 1, 4096, 256, 1048576, temp1, 1048576, B0);
		// add measuring things
	...
	cudaMemcpy(B0_host, B0, 1048576 * sizeof(std::complex<double>), cudaMemcpyDeviceToHost);	
	...
	if(rank == 0)
		printf("4096 4096 32\tD\t16 32\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
