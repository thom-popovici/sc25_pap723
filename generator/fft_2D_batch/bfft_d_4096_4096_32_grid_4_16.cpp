...
void function(int cold_runs, int hot_runs)
{
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 256, 4096, 8, 8388608, A0, 8388608, temp0);
		// add measuring things
		cudaEventRecord(counters[1]);
		pack_data(65536, 16, 8, 8388608, (double*) temp0, 8388608, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[2]);
		MPI_Alltoall((double*) temp1, 8388608/16, MPI_DOUBLE_COMPLEX, (double*) temp0, 8388608/16,  MPI_DOUBLE_COMPLEX, comm1);
		// add measuring things
		cudaEventRecord(counters[3]);
		unpack_data(256, 2048, 16, 8388608, (double*) temp0, 8388608, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[4]);
		fft_compute(CUFFT_FORWARD, fft_plan1, 1, 4096, 2048, 8388608, temp1, 8388608, B0);
		// add measuring things
	...
	if(rank == 0)
		printf("4096 4096 32\tD\t4 16\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
