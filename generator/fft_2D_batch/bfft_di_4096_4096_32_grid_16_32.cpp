...
void function(int cold_runs, int hot_runs)
{
	...
	cufftPlanMany(&fft_plan0, 1, inx, inembed, 1,  4096, onembed, 1,  4096, CUFFT_Z2Z, 256);

	cufftHandle fft_plan1;
	inx[0] = inembed[0] = onembed[0] = 4096;
	cufftPlanMany(&fft_plan1, 1, inx, inembed, 1,  4096, onembed, 1,  4096, CUFFT_Z2Z, 256);

	cufftHandle fft_plan2;
	inx[0] = inembed[0] = onembed[0] = 4096;
	cufftPlanMany(&fft_plan2, 1, inx, inembed, 1,  4096, onembed, 1,  4096, CUFFT_Z2Z, 256);

	cufftHandle fft_plan3;
	inx[0] = inembed[0] = onembed[0] = 4096;
	cufftPlanMany(&fft_plan3, 1, inx, inembed, 1,  4096, onembed, 1,  4096, CUFFT_Z2Z, 256);
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 128, 4096, 2, 1048576, A0, 1048576, temp0);
		// add measuring things
		cudaEventRecord(counters[1]);
		pack_data(16384, 32, 2, 1048576, (double*) temp0, 1048576, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[2]);
		MPI_Alltoall((double*) temp1, 1048576/32, MPI_DOUBLE_COMPLEX, (double*) temp0, 1048576/32,  MPI_DOUBLE_COMPLEX, comm1);
		// add measuring things
		cudaEventRecord(counters[3]);
		unpack_data(128, 256, 32, 1048576, (double*) temp0, 1048576, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[4]);
		fft_compute(CUFFT_FORWARD, fft_plan1, 1, 4096, 256, 1048576, temp1, 1048576, temp0);
		// add measuring things
		cudaEventRecord(counters[5]);
		fft_compute(CUFFT_INVERSE, fft_plan2, 1, 4096, 256, 1048576, temp0, 1048576, temp1);
		// add measuring things
		cudaEventRecord(counters[6]);
		pack_data(128, 32, 256, 1048576, (double*) temp1, 1048576, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[7]);
		MPI_Alltoall((double*) temp0, 1048576/32, MPI_DOUBLE_COMPLEX, (double*) temp1, 1048576/32,  MPI_DOUBLE_COMPLEX, comm1);
		// add measuring things
		cudaEventRecord(counters[8]);
		unpack_data(16384, 2, 32, 1048576, (double*) temp1, 1048576, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[9]);
		fft_compute(CUFFT_INVERSE, fft_plan3, 128, 4096, 2, 1048576, temp0, 1048576, C0);
		// add measuring things
	...
	if(rank == 0)
		printf("4096 4096 32\tDI\t16 32\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
