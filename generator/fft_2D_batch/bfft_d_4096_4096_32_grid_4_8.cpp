...
void function(int cold_runs, int hot_runs)
{
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 1, 4096, 4096, 16777216, A0, 16777216, temp0);
		// add measuring things
		cudaEventRecord(counters[1]);
		pack_data(1024, 4, 4096, 16777216, (double*) temp0, 16777216, (double*) temp1);
		// add measuring things
		...
		unpack_data(1048576, 4, 4, 16777216, (double*) temp0, 16777216, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[4]);
		fft_compute(CUFFT_FORWARD, fft_plan1, 1024, 4096, 4, 16777216, temp1, 16777216, B0);
		// add measuring things
	...
	if(rank == 0)
		printf("4096 4096 32\tD\t4 8\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
