...
void function(int cold_runs, int hot_runs)
{
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 512, 4096, 16, 33554432, A0, 33554432, temp0);
		// add measuring things
		cudaEventRecord(counters[1]);
		pack_data(262144, 8, 16, 33554432, (double*) temp0, 33554432, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[2]);
		MPI_Alltoall((double*) temp1, 33554432/8, MPI_DOUBLE_COMPLEX, (double*) temp0, 33554432/8,  MPI_DOUBLE_COMPLEX, comm1);
		// add measuring things
		cudaEventRecord(counters[3]);
		unpack_data(512, 8192, 8, 33554432, (double*) temp0, 33554432, (double*) temp1);
		// add measuring things
		...
		pack_data(512, 8, 8192, 33554432, (double*) temp1, 33554432, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[7]);
		MPI_Alltoall((double*) temp0, 33554432/8, MPI_DOUBLE_COMPLEX, (double*) temp1, 33554432/8,  MPI_DOUBLE_COMPLEX, comm1);
		// add measuring things
		cudaEventRecord(counters[8]);
		unpack_data(262144, 16, 8, 33554432, (double*) temp1, 33554432, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[9]);
		fft_compute(CUFFT_INVERSE, fft_plan3, 512, 4096, 16, 33554432, temp0, 33554432, C0);
		// add measuring things
	...
	if(rank == 0)
		printf("4096 4096 32\tDI\t2 8\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
