...
void function(int cold_runs, int hot_runs)
{
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 1, 1024, 16384, 16777216, A0, 16777216, temp0);
		// add measuring things
		cudaEventRecord(counters[1]);
		pack_data(256, 4, 16384, 16777216, (double*) temp0, 16777216, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[2]);
		MPI_Alltoall((double*) temp1, 16777216/4, MPI_DOUBLE_COMPLEX, (double*) temp0, 16777216/4,  MPI_DOUBLE_COMPLEX, comm0);
		// add measuring things
		cudaEventRecord(counters[3]);
		unpack_data(65536, 64, 4, 16777216, (double*) temp0, 16777216, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[4]);
		fft_compute(CUFFT_FORWARD, fft_plan1, 256, 1024, 64, 16777216, temp1, 16777216, temp0);
		// add measuring things
		cudaEventRecord(counters[5]);
		fft_compute(CUFFT_INVERSE, fft_plan2, 256, 1024, 64, 16777216, temp0, 16777216, temp1);
		// add measuring things
		cudaEventRecord(counters[6]);
		pack_data(65536, 4, 64, 16777216, (double*) temp1, 16777216, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[7]);
		MPI_Alltoall((double*) temp0, 16777216/4, MPI_DOUBLE_COMPLEX, (double*) temp1, 16777216/4,  MPI_DOUBLE_COMPLEX, comm0);
		// add measuring things
		cudaEventRecord(counters[8]);
		unpack_data(256, 16384, 4, 16777216, (double*) temp1, 16777216, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[9]);
		fft_compute(CUFFT_INVERSE, fft_plan3, 1, 1024, 16384, 16777216, temp0, 16777216, C0);
		// add measuring things
	...
	if(rank == 0)
		printf("1024 1024 512\tDI\t4 8\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
