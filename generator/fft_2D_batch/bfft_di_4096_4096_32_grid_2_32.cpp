...
void function(int cold_runs, int hot_runs)
{
	...
	cufftPlanMany(&fft_plan0, 1, inx, inembed, 1,  4096, onembed, 1,  4096, CUFFT_Z2Z, 2048);

	cufftHandle fft_plan1;
	inx[0] = inembed[0] = onembed[0] = 4096;
	cufftPlanMany(&fft_plan1, 1, inx, inembed, 1,  4096, onembed, 1,  4096, CUFFT_Z2Z, 2048);

	cufftHandle fft_plan2;
	inx[0] = inembed[0] = onembed[0] = 4096;
	cufftPlanMany(&fft_plan2, 1, inx, inembed, 1,  4096, onembed, 1,  4096, CUFFT_Z2Z, 2048);

	cufftHandle fft_plan3;
	inx[0] = inembed[0] = onembed[0] = 4096;
	cufftPlanMany(&fft_plan3, 1, inx, inembed, 1,  4096, onembed, 1,  4096, CUFFT_Z2Z, 2048);
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 2048, 4096, 1, 8388608, A0, 8388608, temp0);
		// add measuring things
		...
		unpack_data(2048, 2048, 2, 8388608, (double*) temp1, 8388608, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[3]);
		fft_compute(CUFFT_FORWARD, fft_plan1, 1, 4096, 2048, 8388608, temp0, 8388608, temp1);
		// add measuring things
		cudaEventRecord(counters[4]);
		fft_compute(CUFFT_INVERSE, fft_plan2, 1, 4096, 2048, 8388608, temp1, 8388608, temp0);
		// add measuring things
		cudaEventRecord(counters[5]);
		pack_data(2048, 2, 2048, 8388608, (double*) temp0, 8388608, (double*) temp1);
		// add measuring things
		...
		fft_compute(CUFFT_INVERSE, fft_plan3, 2048, 4096, 1, 8388608, temp0, 8388608, C0);
		// add measuring things
	...
	if(rank == 0)
		printf("4096 4096 32\tDI\t2 32\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
