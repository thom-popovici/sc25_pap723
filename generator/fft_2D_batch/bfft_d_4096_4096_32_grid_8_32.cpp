...
void function(int cold_runs, int hot_runs)
{
	...
	cudaMemcpy(B0, B0_host, 2097152 * sizeof(std::complex<double>), cudaMemcpyHostToDevice);	
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 128, 4096, 4, 2097152, A0, 2097152, temp0);
		// add measuring things
		cudaEventRecord(counters[1]);
		pack_data(16384, 32, 4, 2097152, (double*) temp0, 2097152, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[2]);
		MPI_Alltoall((double*) temp1, 2097152/32, MPI_DOUBLE_COMPLEX, (double*) temp0, 2097152/32,  MPI_DOUBLE_COMPLEX, comm1);
		// add measuring things
		cudaEventRecord(counters[3]);
		unpack_data(128, 512, 32, 2097152, (double*) temp0, 2097152, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[4]);
		fft_compute(CUFFT_FORWARD, fft_plan1, 1, 4096, 512, 2097152, temp1, 2097152, B0);
		// add measuring things
	...
	cudaMemcpy(B0_host, B0, 2097152 * sizeof(std::complex<double>), cudaMemcpyDeviceToHost);	
	...
	if(rank == 0)
		printf("4096 4096 32\tD\t8 32\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
