...
void function(int cold_runs, int hot_runs)
{
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 128, 1024, 256, 33554432, A0, 33554432, temp0);
		// add measuring things
		cudaEventRecord(counters[1]);
		pack_data(16384, 8, 256, 33554432, (double*) temp0, 33554432, (double*) temp1);
		// add measuring things
		...
		unpack_data(128, 32768, 8, 33554432, (double*) temp0, 33554432, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[4]);
		fft_compute(CUFFT_FORWARD, fft_plan1, 1, 1024, 32768, 33554432, temp1, 33554432, B0);
		// add measuring things
	...
	if(rank == 0)
		printf("1024 1024 512\tD\t2 8\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
