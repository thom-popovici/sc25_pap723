...
void function(int cold_runs, int hot_runs)
{
	...
	A0_host = (std::complex<double>*) malloc(8388608 * sizeof(std::complex<double>));	
	C0_host = (std::complex<double>*) malloc(8388608 * sizeof(std::complex<double>));	
	...
	cudaMalloc((void**)&A0, 8388608 * sizeof(std::complex<double>));	
	cudaMalloc((void**)&C0, 8388608 * sizeof(std::complex<double>));	
	...
	cudaMemcpy(A0, A0_host, 8388608 * sizeof(std::complex<double>), cudaMemcpyHostToDevice);	
	cudaMemcpy(C0, C0_host, 8388608 * sizeof(std::complex<double>), cudaMemcpyHostToDevice);	
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 256, 1024, 32, 8388608, A0, 8388608, temp0);
		// add measuring things
		cudaEventRecord(counters[1]);
		pack_data(65536, 4, 32, 8388608, (double*) temp0, 8388608, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[2]);
		MPI_Alltoall((double*) temp1, 8388608/4, MPI_DOUBLE_COMPLEX, (double*) temp0, 8388608/4,  MPI_DOUBLE_COMPLEX, comm0);
		// add measuring things
		cudaEventRecord(counters[3]);
		unpack_data(256, 8192, 4, 8388608, (double*) temp0, 8388608, (double*) temp1);
		// add measuring things
		...
		pack_data(256, 4, 8192, 8388608, (double*) temp1, 8388608, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[7]);
		MPI_Alltoall((double*) temp0, 8388608/4, MPI_DOUBLE_COMPLEX, (double*) temp1, 8388608/4,  MPI_DOUBLE_COMPLEX, comm0);
		// add measuring things
		cudaEventRecord(counters[8]);
		unpack_data(65536, 32, 4, 8388608, (double*) temp1, 8388608, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[9]);
		fft_compute(CUFFT_INVERSE, fft_plan3, 256, 1024, 32, 8388608, temp0, 8388608, C0);
		// add measuring things
	...
	cudaMemcpy(C0_host, C0, 8388608 * sizeof(std::complex<double>), cudaMemcpyDeviceToHost);	
	...
	if(rank == 0)
		printf("1024 1024 512\tDI\t4 16\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
