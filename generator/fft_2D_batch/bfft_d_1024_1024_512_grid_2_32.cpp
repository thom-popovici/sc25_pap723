...
void function(int cold_runs, int hot_runs)
{
	...
	cudaMemcpy(B0, B0_host, 8388608 * sizeof(std::complex<double>), cudaMemcpyHostToDevice);	
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 1, 1024, 8192, 8388608, A0, 8388608, temp0);
		// add measuring things
		cudaEventRecord(counters[1]);
		pack_data(32, 32, 8192, 8388608, (double*) temp0, 8388608, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[2]);
		MPI_Alltoall((double*) temp1, 8388608/32, MPI_DOUBLE_COMPLEX, (double*) temp0, 8388608/32,  MPI_DOUBLE_COMPLEX, comm1);
		// add measuring things
		cudaEventRecord(counters[3]);
		unpack_data(1024, 256, 32, 8388608, (double*) temp0, 8388608, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[4]);
		fft_compute(CUFFT_FORWARD, fft_plan1, 32, 1024, 256, 8388608, temp1, 8388608, B0);
		// add measuring things
	...
	cudaMemcpy(B0_host, B0, 8388608 * sizeof(std::complex<double>), cudaMemcpyDeviceToHost);	
	...
	if(rank == 0)
		printf("1024 1024 512\tD\t2 32\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
