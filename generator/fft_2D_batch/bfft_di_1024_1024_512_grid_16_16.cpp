...
void function(int cold_runs, int hot_runs)
{
	...
	A0_host = (std::complex<double>*) malloc(2097152 * sizeof(std::complex<double>));	
	C0_host = (std::complex<double>*) malloc(2097152 * sizeof(std::complex<double>));	
	...
	cudaMalloc((void**)&A0, 2097152 * sizeof(std::complex<double>));	
	cudaMalloc((void**)&C0, 2097152 * sizeof(std::complex<double>));	
	...
	cudaMemcpy(A0, A0_host, 2097152 * sizeof(std::complex<double>), cudaMemcpyHostToDevice);	
	cudaMemcpy(C0, C0_host, 2097152 * sizeof(std::complex<double>), cudaMemcpyHostToDevice);	
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 1, 1024, 2048, 2097152, A0, 2097152, temp0);
		// add measuring things
		cudaEventRecord(counters[1]);
		pack_data(64, 16, 2048, 2097152, (double*) temp0, 2097152, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[2]);
		MPI_Alltoall((double*) temp1, 2097152/16, MPI_DOUBLE_COMPLEX, (double*) temp0, 2097152/16,  MPI_DOUBLE_COMPLEX, comm1);
		// add measuring things
		cudaEventRecord(counters[3]);
		unpack_data(4096, 32, 16, 2097152, (double*) temp0, 2097152, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[4]);
		fft_compute(CUFFT_FORWARD, fft_plan1, 64, 1024, 32, 2097152, temp1, 2097152, temp0);
		// add measuring things
		cudaEventRecord(counters[5]);
		fft_compute(CUFFT_INVERSE, fft_plan2, 64, 1024, 32, 2097152, temp0, 2097152, temp1);
		// add measuring things
		cudaEventRecord(counters[6]);
		pack_data(4096, 16, 32, 2097152, (double*) temp1, 2097152, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[7]);
		MPI_Alltoall((double*) temp0, 2097152/16, MPI_DOUBLE_COMPLEX, (double*) temp1, 2097152/16,  MPI_DOUBLE_COMPLEX, comm1);
		// add measuring things
		cudaEventRecord(counters[8]);
		unpack_data(64, 2048, 16, 2097152, (double*) temp1, 2097152, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[9]);
		fft_compute(CUFFT_INVERSE, fft_plan3, 1, 1024, 2048, 2097152, temp0, 2097152, C0);
		// add measuring things
	...
	cudaMemcpy(C0_host, C0, 2097152 * sizeof(std::complex<double>), cudaMemcpyDeviceToHost);	
	...
	if(rank == 0)
		printf("1024 1024 512\tDI\t16 16\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
