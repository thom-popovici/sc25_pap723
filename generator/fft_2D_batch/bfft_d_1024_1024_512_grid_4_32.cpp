...
void function(int cold_runs, int hot_runs)
{
	...
	cudaMemcpy(B0, B0_host, 4194304 * sizeof(std::complex<double>), cudaMemcpyHostToDevice);	
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 32, 1024, 128, 4194304, A0, 4194304, temp0);
		// add measuring things
		cudaEventRecord(counters[1]);
		pack_data(1024, 32, 128, 4194304, (double*) temp0, 4194304, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[2]);
		MPI_Alltoall((double*) temp1, 4194304/32, MPI_DOUBLE_COMPLEX, (double*) temp0, 4194304/32,  MPI_DOUBLE_COMPLEX, comm1);
		// add measuring things
		cudaEventRecord(counters[3]);
		unpack_data(32, 4096, 32, 4194304, (double*) temp0, 4194304, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[4]);
		fft_compute(CUFFT_FORWARD, fft_plan1, 1, 1024, 4096, 4194304, temp1, 4194304, B0);
		// add measuring things
	...
	cudaMemcpy(B0_host, B0, 4194304 * sizeof(std::complex<double>), cudaMemcpyDeviceToHost);	
	...
	if(rank == 0)
		printf("1024 1024 512\tD\t4 32\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
