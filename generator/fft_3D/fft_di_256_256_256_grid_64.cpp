...
void function(int cold_runs, int hot_runs)
{
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 1, 256, 1024, 262144, A0, 262144, temp0);
		// add measuring things
		cudaEventRecord(counters[1]);
		fft_compute(CUFFT_FORWARD, fft_plan1, 1024, 256, 1, 262144, temp0, 262144, temp1);
		// add measuring things
		cudaEventRecord(counters[2]);
		pack_data(4, 64, 1024, 262144, (double*) temp1, 262144, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[3]);
		MPI_Alltoall((double*) temp0, 262144/64, MPI_DOUBLE_COMPLEX, (double*) temp1, 262144/64,  MPI_DOUBLE_COMPLEX, comm0);
		// add measuring things
		cudaEventRecord(counters[4]);
		unpack_data(16, 256, 64, 262144, (double*) temp1, 262144, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[5]);
		fft_compute(CUFFT_FORWARD, fft_plan2, 4, 256, 256, 262144, temp0, 262144, temp1);
		// add measuring things
		cudaEventRecord(counters[6]);
		fft_compute(CUFFT_INVERSE, fft_plan3, 4, 256, 256, 262144, temp1, 262144, temp0);
		// add measuring things
		...
		MPI_Alltoall((double*) temp1, 262144/64, MPI_DOUBLE_COMPLEX, (double*) temp0, 262144/64,  MPI_DOUBLE_COMPLEX, comm0);
		// add measuring things
		cudaEventRecord(counters[9]);
		unpack_data(4, 1024, 64, 262144, (double*) temp0, 262144, (double*) temp1);
		// add measuring things
