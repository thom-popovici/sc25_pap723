...
void function(int cold_runs, int hot_runs)
{
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 8, 256, 256, 524288, A0, 524288, temp0);
		// add measuring things
		cudaEventRecord(counters[1]);
		fft_compute(CUFFT_FORWARD, fft_plan1, 2048, 256, 1, 524288, temp0, 524288, temp1);
		// add measuring things
		cudaEventRecord(counters[2]);
		MPI_Alltoall((double*) temp1, 524288/32, MPI_DOUBLE_COMPLEX, (double*) temp0, 524288/32,  MPI_DOUBLE_COMPLEX, comm0);
		// add measuring things
		cudaEventRecord(counters[3]);
		unpack_data(8, 2048, 32, 524288, (double*) temp0, 524288, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[4]);
		fft_compute(CUFFT_FORWARD, fft_plan2, 1, 256, 2048, 524288, temp1, 524288, temp0);
		// add measuring things
		cudaEventRecord(counters[5]);
		fft_compute(CUFFT_INVERSE, fft_plan3, 1, 256, 2048, 524288, temp0, 524288, temp1);
		// add measuring things
		cudaEventRecord(counters[6]);
		fft_compute(CUFFT_INVERSE, fft_plan4, 256, 256, 8, 524288, temp1, 524288, temp0);
		// add measuring things
		cudaEventRecord(counters[7]);
		pack_data(2048, 32, 8, 524288, (double*) temp0, 524288, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[8]);
		MPI_Alltoall((double*) temp1, 524288/32, MPI_DOUBLE_COMPLEX, (double*) temp0, 524288/32,  MPI_DOUBLE_COMPLEX, comm0);
		// add measuring things
		cudaEventRecord(counters[9]);
		fft_compute(CUFFT_INVERSE, fft_plan5, 2048, 256, 1, 524288, temp0, 524288, C0);
		// add measuring things
