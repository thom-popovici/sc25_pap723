...
void function(int cold_runs, int hot_runs)
{
	...
	A0_host = (std::complex<double>*) malloc(32768 * sizeof(std::complex<double>));	
	C0_host = (std::complex<double>*) malloc(32768 * sizeof(std::complex<double>));	
	...
	cufftPlanMany(&fft_plan0, 1, inx, inembed, 1,  256, onembed, 1,  256, CUFFT_Z2Z, 128);

	cufftHandle fft_plan1;
	inx[0] = inembed[0] = onembed[0] = 256;
	cufftPlanMany(&fft_plan1, 1, inx, inembed, 1,  256, onembed, 1,  256, CUFFT_Z2Z, 128);

	cufftHandle fft_plan2;
	inx[0] = inembed[0] = onembed[0] = 256;
	cufftPlanMany(&fft_plan2, 1, inx, inembed, 1,  256, onembed, 1,  256, CUFFT_Z2Z, 128);

	cufftHandle fft_plan3;
	inx[0] = inembed[0] = onembed[0] = 256;
	cufftPlanMany(&fft_plan3, 1, inx, inembed, 1,  256, onembed, 1,  256, CUFFT_Z2Z, 128);

	cufftHandle fft_plan4;
	inx[0] = inembed[0] = onembed[0] = 256;
	cufftPlanMany(&fft_plan4, 1, inx, inembed, 1,  256, onembed, 1,  256, CUFFT_Z2Z, 128);

	cufftHandle fft_plan5;
	inx[0] = inembed[0] = onembed[0] = 256;
	cufftPlanMany(&fft_plan5, 1, inx, inembed, 1,  256, onembed, 1,  256, CUFFT_Z2Z, 128);

	// create device arrays
	std::complex<double> *A0 = NULL, *C0 = NULL, *temp = NULL;
	cudaMalloc((void**)&A0, 32768 * sizeof(std::complex<double>));	
	cudaMalloc((void**)&C0, 32768 * sizeof(std::complex<double>));	
	cudaMalloc((void**)&temp, 65536 * sizeof(std::complex<double>));	
	std::complex<double> *temp0 = (temp + 0 * 32768);
	std::complex<double> *temp1 = (temp + 1 * 32768);

	// copy data from arrays to device arrays
	cudaMemcpy(A0, A0_host, 32768 * sizeof(std::complex<double>), cudaMemcpyHostToDevice);	
	cudaMemcpy(C0, C0_host, 32768 * sizeof(std::complex<double>), cudaMemcpyHostToDevice);	
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 128, 256, 1, 32768, A0, 32768, temp0);
		// add measuring things
		cudaEventRecord(counters[1]);
		MPI_Alltoall((double*) temp0, 32768/64, MPI_DOUBLE_COMPLEX, (double*) temp1, 32768/64,  MPI_DOUBLE_COMPLEX, comm1);
		// add measuring things
		cudaEventRecord(counters[2]);
		unpack_data(4, 128, 64, 32768, (double*) temp1, 32768, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[3]);
		fft_compute(CUFFT_FORWARD, fft_plan1, 1, 256, 128, 32768, temp0, 32768, temp1);
		// add measuring things
		cudaEventRecord(counters[4]);
		pack_data(32, 8, 128, 32768, (double*) temp1, 32768, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[5]);
		MPI_Alltoall((double*) temp0, 32768/8, MPI_DOUBLE_COMPLEX, (double*) temp1, 32768/8,  MPI_DOUBLE_COMPLEX, comm0);
		// add measuring things
		cudaEventRecord(counters[6]);
		unpack_data(1024, 4, 8, 32768, (double*) temp1, 32768, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[7]);
		fft_compute(CUFFT_FORWARD, fft_plan2, 32, 256, 4, 32768, temp0, 32768, temp1);
		// add measuring things
		cudaEventRecord(counters[8]);
		fft_compute(CUFFT_INVERSE, fft_plan3, 32, 256, 4, 32768, temp1, 32768, temp0);
		// add measuring things
		cudaEventRecord(counters[9]);
		pack_data(128, 64, 4, 32768, (double*) temp0, 32768, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[10]);
		MPI_Alltoall((double*) temp1, 32768/64, MPI_DOUBLE_COMPLEX, (double*) temp0, 32768/64,  MPI_DOUBLE_COMPLEX, comm1);
		// add measuring things
		cudaEventRecord(counters[11]);
		fft_compute(CUFFT_INVERSE, fft_plan4, 128, 256, 1, 32768, temp0, 32768, temp1);
		// add measuring things
		cudaEventRecord(counters[12]);
		MPI_Alltoall((double*) temp1, 32768/8, MPI_DOUBLE_COMPLEX, (double*) temp0, 32768/8,  MPI_DOUBLE_COMPLEX, comm0);
		// add measuring things
		cudaEventRecord(counters[13]);
		unpack_data(32, 128, 8, 32768, (double*) temp0, 32768, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[14]);
		fft_compute(CUFFT_INVERSE, fft_plan5, 1, 256, 128, 32768, temp1, 32768, C0);
		// add measuring things
	...
	cudaMemcpy(C0_host, C0, 32768 * sizeof(std::complex<double>), cudaMemcpyDeviceToHost);	
	...
	if(rank == 0)
		printf("256 256 256\tDI\t8 64\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
