...
void function(int cold_runs, int hot_runs)
{
	...
	C0_host = (std::complex<double>*) malloc(524288 * sizeof(std::complex<double>));	
	...
	cufftPlanMany(&fft_plan0, 1, inx, inembed, 1,  256, onembed, 1,  256, CUFFT_Z2Z, 2048);

	cufftHandle fft_plan1;
	inx[0] = inembed[0] = onembed[0] = 256;
	cufftPlanMany(&fft_plan1, 1, inx, inembed, 1,  256, onembed, 1,  256, CUFFT_Z2Z, 2048);

	cufftHandle fft_plan2;
	inx[0] = inembed[0] = onembed[0] = 256;
	cufftPlanMany(&fft_plan2, 1, inx, inembed, 1,  256, onembed, 1,  256, CUFFT_Z2Z, 2048);

	cufftHandle fft_plan3;
	inx[0] = inembed[0] = onembed[0] = 256;
	cufftPlanMany(&fft_plan3, 1, inx, inembed, 1,  256, onembed, 1,  256, CUFFT_Z2Z, 2048);

	cufftHandle fft_plan4;
	inx[0] = inembed[0] = onembed[0] = 256;
	cufftPlanMany(&fft_plan4, 1, inx, inembed, 1,  256, onembed, 1,  256, CUFFT_Z2Z, 2048);

	cufftHandle fft_plan5;
	inx[0] = inembed[0] = onembed[0] = 256;
	cufftPlanMany(&fft_plan5, 1, inx, inembed, 1,  256, onembed, 1,  256, CUFFT_Z2Z, 2048);

	// create device arrays
	std::complex<double> *A0 = NULL, *C0 = NULL, *temp = NULL;
	cudaMalloc((void**)&A0, 524288 * sizeof(std::complex<double>));	
	cudaMalloc((void**)&C0, 524288 * sizeof(std::complex<double>));	
	...
	cudaMemcpy(C0, C0_host, 524288 * sizeof(std::complex<double>), cudaMemcpyHostToDevice);	
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 2048, 256, 1, 524288, A0, 524288, temp0);
		// add measuring things
		cudaEventRecord(counters[1]);
		MPI_Alltoall((double*) temp0, 524288/8, MPI_DOUBLE_COMPLEX, (double*) temp1, 524288/8,  MPI_DOUBLE_COMPLEX, comm1);
		// add measuring things
		cudaEventRecord(counters[2]);
		unpack_data(32, 2048, 8, 524288, (double*) temp1, 524288, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[3]);
		fft_compute(CUFFT_FORWARD, fft_plan1, 1, 256, 2048, 524288, temp0, 524288, temp1);
		// add measuring things
		cudaEventRecord(counters[4]);
		pack_data(64, 4, 2048, 524288, (double*) temp1, 524288, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[5]);
		MPI_Alltoall((double*) temp0, 524288/4, MPI_DOUBLE_COMPLEX, (double*) temp1, 524288/4,  MPI_DOUBLE_COMPLEX, comm0);
		// add measuring things
		cudaEventRecord(counters[6]);
		unpack_data(4096, 32, 4, 524288, (double*) temp1, 524288, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[7]);
		fft_compute(CUFFT_FORWARD, fft_plan2, 64, 256, 32, 524288, temp0, 524288, temp1);
		// add measuring things
		cudaEventRecord(counters[8]);
		fft_compute(CUFFT_INVERSE, fft_plan3, 64, 256, 32, 524288, temp1, 524288, temp0);
		// add measuring things
		cudaEventRecord(counters[9]);
		pack_data(2048, 8, 32, 524288, (double*) temp0, 524288, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[10]);
		MPI_Alltoall((double*) temp1, 524288/8, MPI_DOUBLE_COMPLEX, (double*) temp0, 524288/8,  MPI_DOUBLE_COMPLEX, comm1);
		// add measuring things
		cudaEventRecord(counters[11]);
		fft_compute(CUFFT_INVERSE, fft_plan4, 2048, 256, 1, 524288, temp0, 524288, temp1);
		// add measuring things
		cudaEventRecord(counters[12]);
		MPI_Alltoall((double*) temp1, 524288/4, MPI_DOUBLE_COMPLEX, (double*) temp0, 524288/4,  MPI_DOUBLE_COMPLEX, comm0);
		// add measuring things
		cudaEventRecord(counters[13]);
		unpack_data(64, 2048, 4, 524288, (double*) temp0, 524288, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[14]);
		fft_compute(CUFFT_INVERSE, fft_plan5, 1, 256, 2048, 524288, temp1, 524288, C0);
		// add measuring things
	...
	cudaMemcpy(C0_host, C0, 524288 * sizeof(std::complex<double>), cudaMemcpyDeviceToHost);	
	...
	if(rank == 0)
		printf("256 256 256\tDI\t4 8\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
