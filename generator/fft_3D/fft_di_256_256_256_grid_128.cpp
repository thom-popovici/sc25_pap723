...
void function(int cold_runs, int hot_runs)
{
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 2, 256, 256, 131072, A0, 131072, temp0);
		// add measuring things
		cudaEventRecord(counters[1]);
		fft_compute(CUFFT_FORWARD, fft_plan1, 512, 256, 1, 131072, temp0, 131072, temp1);
		// add measuring things
		cudaEventRecord(counters[2]);
		MPI_Alltoall((double*) temp1, 131072/128, MPI_DOUBLE_COMPLEX, (double*) temp0, 131072/128,  MPI_DOUBLE_COMPLEX, comm0);
		// add measuring things
		cudaEventRecord(counters[3]);
		unpack_data(2, 512, 128, 131072, (double*) temp0, 131072, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[4]);
		fft_compute(CUFFT_FORWARD, fft_plan2, 1, 256, 512, 131072, temp1, 131072, temp0);
		// add measuring things
		cudaEventRecord(counters[5]);
		fft_compute(CUFFT_INVERSE, fft_plan3, 1, 256, 512, 131072, temp0, 131072, temp1);
		// add measuring things
		cudaEventRecord(counters[6]);
		fft_compute(CUFFT_INVERSE, fft_plan4, 256, 256, 2, 131072, temp1, 131072, temp0);
		// add measuring things
		cudaEventRecord(counters[7]);
		pack_data(512, 128, 2, 131072, (double*) temp0, 131072, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[8]);
		MPI_Alltoall((double*) temp1, 131072/128, MPI_DOUBLE_COMPLEX, (double*) temp0, 131072/128,  MPI_DOUBLE_COMPLEX, comm0);
		// add measuring things
		cudaEventRecord(counters[9]);
		fft_compute(CUFFT_INVERSE, fft_plan5, 512, 256, 1, 131072, temp0, 131072, C0);
		// add measuring things
	...
	if(rank == 0)
		printf("256 256 256\tDI\t128\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
