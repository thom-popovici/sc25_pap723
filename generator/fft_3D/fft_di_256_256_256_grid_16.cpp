...
void function(int cold_runs, int hot_runs)
{
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 16, 256, 256, 1048576, A0, 1048576, temp0);
		// add measuring things
		cudaEventRecord(counters[1]);
		fft_compute(CUFFT_FORWARD, fft_plan1, 4096, 256, 1, 1048576, temp0, 1048576, temp1);
		// add measuring things
		...
		unpack_data(16, 4096, 16, 1048576, (double*) temp0, 1048576, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[4]);
		fft_compute(CUFFT_FORWARD, fft_plan2, 1, 256, 4096, 1048576, temp1, 1048576, temp0);
		// add measuring things
		cudaEventRecord(counters[5]);
		fft_compute(CUFFT_INVERSE, fft_plan3, 1, 256, 4096, 1048576, temp0, 1048576, temp1);
		// add measuring things
		cudaEventRecord(counters[6]);
		fft_compute(CUFFT_INVERSE, fft_plan4, 256, 256, 16, 1048576, temp1, 1048576, temp0);
		// add measuring things
		cudaEventRecord(counters[7]);
		pack_data(4096, 16, 16, 1048576, (double*) temp0, 1048576, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[8]);
		MPI_Alltoall((double*) temp1, 1048576/16, MPI_DOUBLE_COMPLEX, (double*) temp0, 1048576/16,  MPI_DOUBLE_COMPLEX, comm0);
		// add measuring things
		cudaEventRecord(counters[9]);
		fft_compute(CUFFT_INVERSE, fft_plan5, 4096, 256, 1, 1048576, temp0, 1048576, C0);
		// add measuring things
