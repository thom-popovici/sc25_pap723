...
void function(int cold_runs, int hot_runs)
{
	...
	C0_host = (std::complex<double>*) malloc(33554432 * sizeof(std::complex<double>));	
	...
	cudaMalloc((void**)&C0, 33554432 * sizeof(std::complex<double>));	
	...
	cudaMemcpy(C0, C0_host, 33554432 * sizeof(std::complex<double>), cudaMemcpyHostToDevice);	
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 1, 256, 131072, 33554432, A0, 33554432, temp0);
		// add measuring things
		cudaEventRecord(counters[1]);
		pack_data(64, 4, 131072, 33554432, (double*) temp0, 33554432, (double*) temp1);
		// add measuring things
		...
		unpack_data(4096, 2048, 4, 33554432, (double*) temp0, 33554432, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[4]);
		fft_compute(CUFFT_FORWARD, fft_plan1, 64, 256, 2048, 33554432, temp1, 33554432, temp0);
		// add measuring things
		cudaEventRecord(counters[5]);
		pack_data(256, 64, 2048, 33554432, (double*) temp0, 33554432, (double*) temp1);
		// add measuring things
		...
		unpack_data(1024, 512, 64, 33554432, (double*) temp0, 33554432, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[8]);
		fft_compute(CUFFT_FORWARD, fft_plan2, 256, 256, 512, 33554432, temp1, 33554432, temp0);
		// add measuring things
		cudaEventRecord(counters[9]);
		fft_compute(CUFFT_INVERSE, fft_plan3, 256, 256, 512, 33554432, temp0, 33554432, temp1);
		// add measuring things
		cudaEventRecord(counters[10]);
		pack_data(16384, 4, 512, 33554432, (double*) temp1, 33554432, (double*) temp0);
		// add measuring things
		...
		unpack_data(64, 131072, 4, 33554432, (double*) temp1, 33554432, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[13]);
		fft_compute(CUFFT_INVERSE, fft_plan4, 1, 256, 131072, 33554432, temp0, 33554432, temp1);
		// add measuring things
		cudaEventRecord(counters[14]);
		pack_data(4, 64, 131072, 33554432, (double*) temp1, 33554432, (double*) temp0);
		// add measuring things
		...
		unpack_data(16, 32768, 64, 33554432, (double*) temp1, 33554432, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[17]);
		fft_compute(CUFFT_INVERSE, fft_plan5, 4, 256, 32768, 33554432, temp0, 33554432, C0);
		// add measuring things
	...
	cudaMemcpy(C0_host, C0, 33554432 * sizeof(std::complex<double>), cudaMemcpyDeviceToHost);	
	...
	if(rank == 0)
		printf("256 256 256 1024\tDI\t2 4 64\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
