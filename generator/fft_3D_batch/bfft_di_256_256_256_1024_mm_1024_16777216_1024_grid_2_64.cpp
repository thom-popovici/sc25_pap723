...
void function(int cold_runs, int hot_runs)
{
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 1, 256, 524288, 134217728, A0, 134217728, temp0);
		// add measuring things
		cudaEventRecord(counters[1]);
		fft_compute(CUFFT_FORWARD, fft_plan1, 1024, 256, 512, 134217728, temp0, 134217728, temp1);
		// add measuring things
		cudaEventRecord(counters[2]);
		pack_data(4, 64, 524288, 134217728, (double*) temp1, 134217728, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[3]);
		MPI_Alltoall((double*) temp0, 134217728/64, MPI_DOUBLE_COMPLEX, (double*) temp1, 134217728/64,  MPI_DOUBLE_COMPLEX, comm1);
		// add measuring things
		cudaEventRecord(counters[4]);
		unpack_data(16, 131072, 64, 134217728, (double*) temp1, 134217728, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[5]);
		fft_compute(CUFFT_FORWARD, fft_plan2, 4, 256, 131072, 134217728, temp0, 134217728, temp1);
		// add measuring things
		cudaEventRecord(counters[6]);
		fft_compute(CUFFT_INVERSE, fft_plan3, 4, 256, 131072, 134217728, temp1, 134217728, temp0);
		// add measuring things
		...
		pack_data(4096, 64, 512, 134217728, (double*) temp1, 134217728, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[9]);
		MPI_Alltoall((double*) temp0, 134217728/64, MPI_DOUBLE_COMPLEX, (double*) temp1, 134217728/64,  MPI_DOUBLE_COMPLEX, comm1);
		// add measuring things
		cudaEventRecord(counters[10]);
		unpack_data(4, 524288, 64, 134217728, (double*) temp1, 134217728, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[11]);
		fft_compute(CUFFT_INVERSE, fft_plan5, 1, 256, 524288, 134217728, temp0, 134217728, temp1);
		// add measuring things
		cudaEventRecord(counters[12]);
		pack_data(1024, 64, 2048, 134217728, (double*) temp1, 134217728, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[13]);
		MPI_Alltoall((double*) temp0, 134217728/64, MPI_DOUBLE_COMPLEX, (double*) temp1, 134217728/64,  MPI_DOUBLE_COMPLEX, comm1);
		// add measuring things
		cudaEventRecord(counters[14]);
		unpack_data(4096, 512, 64, 134217728, (double*) temp1, 134217728, (double*) C0);
		// add measuring things
	...
	if(rank == 0)
		printf("256 256 256 1024\tDI\t2 64\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
