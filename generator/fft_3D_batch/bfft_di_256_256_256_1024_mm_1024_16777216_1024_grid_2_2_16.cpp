...
void function(int cold_runs, int hot_runs)
{
	...
	C0_host = (std::complex<double>*) malloc(268435456 * sizeof(std::complex<double>));	
	...
	cudaMalloc((void**)&C0, 268435456 * sizeof(std::complex<double>));	
	...
	cudaMemcpy(C0, C0_host, 268435456 * sizeof(std::complex<double>), cudaMemcpyHostToDevice);	
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 128, 256, 8192, 268435456, A0, 268435456, temp0);
		// add measuring things
		cudaEventRecord(counters[1]);
		pack_data(16384, 2, 8192, 268435456, (double*) temp0, 268435456, (double*) temp1);
		// add measuring things
		...
		unpack_data(2097152, 64, 2, 268435456, (double*) temp0, 268435456, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[4]);
		fft_compute(CUFFT_FORWARD, fft_plan1, 16384, 256, 64, 268435456, temp1, 268435456, temp0);
		// add measuring things
		cudaEventRecord(counters[5]);
		pack_data(2097152, 2, 64, 268435456, (double*) temp0, 268435456, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[6]);
		MPI_Alltoall((double*) temp1, 268435456/2, MPI_DOUBLE_COMPLEX, (double*) temp0, 268435456/2,  MPI_DOUBLE_COMPLEX, comm1);
		// add measuring things
		cudaEventRecord(counters[7]);
		unpack_data(128, 1048576, 2, 268435456, (double*) temp0, 268435456, (double*) temp1);
		// add measuring things
		...
		pack_data(16384, 2, 8192, 268435456, (double*) temp1, 268435456, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[15]);
		MPI_Alltoall((double*) temp0, 268435456/2, MPI_DOUBLE_COMPLEX, (double*) temp1, 268435456/2,  MPI_DOUBLE_COMPLEX, comm1);
		// add measuring things
		cudaEventRecord(counters[16]);
		unpack_data(2097152, 64, 2, 268435456, (double*) temp1, 268435456, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[17]);
		fft_compute(CUFFT_INVERSE, fft_plan5, 16384, 256, 64, 268435456, temp0, 268435456, C0);
		// add measuring things
	...
	cudaMemcpy(C0_host, C0, 268435456 * sizeof(std::complex<double>), cudaMemcpyDeviceToHost);	
	...
	if(rank == 0)
		printf("256 256 256 1024\tDI\t2 2 16\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
