...
void function(int cold_runs, int hot_runs)
{
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 1, 256, 262144, 67108864, A0, 67108864, temp0);
		// add measuring things
		cudaEventRecord(counters[1]);
		fft_compute(CUFFT_FORWARD, fft_plan1, 4096, 256, 64, 67108864, temp0, 67108864, temp1);
		// add measuring things
		cudaEventRecord(counters[2]);
		pack_data(16, 16, 262144, 67108864, (double*) temp1, 67108864, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[3]);
		MPI_Alltoall((double*) temp0, 67108864/16, MPI_DOUBLE_COMPLEX, (double*) temp1, 67108864/16,  MPI_DOUBLE_COMPLEX, comm1);
		// add measuring things
		cudaEventRecord(counters[4]);
		unpack_data(256, 16384, 16, 67108864, (double*) temp1, 67108864, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[5]);
		fft_compute(CUFFT_FORWARD, fft_plan2, 16, 256, 16384, 67108864, temp0, 67108864, temp1);
		// add measuring things
		cudaEventRecord(counters[6]);
		fft_compute(CUFFT_INVERSE, fft_plan3, 16, 256, 16384, 67108864, temp1, 67108864, temp0);
		// add measuring things
		cudaEventRecord(counters[7]);
		fft_compute(CUFFT_INVERSE, fft_plan4, 4096, 256, 64, 67108864, temp0, 67108864, temp1);
		// add measuring things
		cudaEventRecord(counters[8]);
		pack_data(65536, 16, 64, 67108864, (double*) temp1, 67108864, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[9]);
		MPI_Alltoall((double*) temp0, 67108864/16, MPI_DOUBLE_COMPLEX, (double*) temp1, 67108864/16,  MPI_DOUBLE_COMPLEX, comm1);
		// add measuring things
		cudaEventRecord(counters[10]);
		unpack_data(16, 262144, 16, 67108864, (double*) temp1, 67108864, (double*) temp0);
		// add measuring things
	...
	if(rank == 0)
		printf("256 256 256 1024\tDI\t16 16\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
