...
void function(int cold_runs, int hot_runs)
{
	...
	C0_host = (std::complex<double>*) malloc(134217728 * sizeof(std::complex<double>));	
	...
	cudaMalloc((void**)&C0, 134217728 * sizeof(std::complex<double>));	
	...
	cudaMemcpy(C0, C0_host, 134217728 * sizeof(std::complex<double>), cudaMemcpyHostToDevice);	
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 1024, 256, 512, 134217728, A0, 134217728, temp0);
		// add measuring things
		cudaEventRecord(counters[1]);
		pack_data(65536, 4, 512, 134217728, (double*) temp0, 134217728, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[2]);
		MPI_Alltoall((double*) temp1, 134217728/4, MPI_DOUBLE_COMPLEX, (double*) temp0, 134217728/4,  MPI_DOUBLE_COMPLEX, comm1);
		// add measuring things
		cudaEventRecord(counters[3]);
		unpack_data(64, 524288, 4, 134217728, (double*) temp0, 134217728, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[4]);
		fft_compute(CUFFT_FORWARD, fft_plan1, 1, 256, 524288, 134217728, temp1, 134217728, temp0);
		// add measuring things
		cudaEventRecord(counters[5]);
		pack_data(16, 16, 524288, 134217728, (double*) temp0, 134217728, (double*) temp1);
		// add measuring things
		...
		unpack_data(256, 32768, 16, 134217728, (double*) temp0, 134217728, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[8]);
		fft_compute(CUFFT_FORWARD, fft_plan2, 16, 256, 32768, 134217728, temp1, 134217728, temp0);
		// add measuring things
		cudaEventRecord(counters[9]);
		fft_compute(CUFFT_INVERSE, fft_plan3, 16, 256, 32768, 134217728, temp0, 134217728, temp1);
		// add measuring things
		cudaEventRecord(counters[10]);
		pack_data(1024, 4, 32768, 134217728, (double*) temp1, 134217728, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[11]);
		MPI_Alltoall((double*) temp0, 134217728/4, MPI_DOUBLE_COMPLEX, (double*) temp1, 134217728/4,  MPI_DOUBLE_COMPLEX, comm1);
		// add measuring things
		cudaEventRecord(counters[12]);
		unpack_data(65536, 512, 4, 134217728, (double*) temp1, 134217728, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[13]);
		fft_compute(CUFFT_INVERSE, fft_plan4, 1024, 256, 512, 134217728, temp0, 134217728, temp1);
		// add measuring things
		cudaEventRecord(counters[14]);
		pack_data(16384, 16, 512, 134217728, (double*) temp1, 134217728, (double*) temp0);
		// add measuring things
		...
		unpack_data(16, 524288, 16, 134217728, (double*) temp1, 134217728, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[17]);
		fft_compute(CUFFT_INVERSE, fft_plan5, 1, 256, 524288, 134217728, temp0, 134217728, C0);
		// add measuring things
	...
	cudaMemcpy(C0_host, C0, 134217728 * sizeof(std::complex<double>), cudaMemcpyDeviceToHost);	
	...
	if(rank == 0)
		printf("256 256 256 1024\tDI\t2 4 16\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
