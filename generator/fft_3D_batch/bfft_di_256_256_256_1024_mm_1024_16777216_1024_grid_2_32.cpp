...
void function(int cold_runs, int hot_runs)
{
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 1, 256, 1048576, 268435456, A0, 268435456, temp0);
		// add measuring things
		cudaEventRecord(counters[1]);
		fft_compute(CUFFT_FORWARD, fft_plan1, 32768, 256, 32, 268435456, temp0, 268435456, temp1);
		// add measuring things
		...
		fft_compute(CUFFT_FORWARD, fft_plan2, 128, 256, 8192, 268435456, temp0, 268435456, temp1);
		// add measuring things
		cudaEventRecord(counters[6]);
		fft_compute(CUFFT_INVERSE, fft_plan3, 128, 256, 8192, 268435456, temp1, 268435456, temp0);
		// add measuring things
		cudaEventRecord(counters[7]);
		fft_compute(CUFFT_INVERSE, fft_plan4, 32768, 256, 32, 268435456, temp0, 268435456, temp1);
		// add measuring things
		cudaEventRecord(counters[8]);
		pack_data(4194304, 2, 32, 268435456, (double*) temp1, 268435456, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[9]);
		MPI_Alltoall((double*) temp0, 268435456/2, MPI_DOUBLE_COMPLEX, (double*) temp1, 268435456/2,  MPI_DOUBLE_COMPLEX, comm0);
		// add measuring things
		cudaEventRecord(counters[10]);
		unpack_data(128, 1048576, 2, 268435456, (double*) temp1, 268435456, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[11]);
		fft_compute(CUFFT_INVERSE, fft_plan5, 1, 256, 1048576, 268435456, temp0, 268435456, C0);
		// add measuring things
	...
	if(rank == 0)
		printf("256 256 256 1024\tDI\t2 32\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
