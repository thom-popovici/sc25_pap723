...
void function(int cold_runs, int hot_runs)
{
	...
	cufftPlanMany(&fft_plan0, 1, inx, inembed, 1,  256, onembed, 1,  256, CUFFT_Z2Z, 131072);

	cufftHandle fft_plan1;
	inx[0] = inembed[0] = onembed[0] = 256;
	cufftPlanMany(&fft_plan1, 1, inx, inembed, 1,  256, onembed, 1,  256, CUFFT_Z2Z, 131072);

	cufftHandle fft_plan2;
	inx[0] = inembed[0] = onembed[0] = 256;
	cufftPlanMany(&fft_plan2, 1, inx, inembed, 1,  256, onembed, 1,  256, CUFFT_Z2Z, 131072);

	cufftHandle fft_plan3;
	inx[0] = inembed[0] = onembed[0] = 256;
	cufftPlanMany(&fft_plan3, 1, inx, inembed, 1,  256, onembed, 1,  256, CUFFT_Z2Z, 131072);

	cufftHandle fft_plan4;
	inx[0] = inembed[0] = onembed[0] = 256;
	cufftPlanMany(&fft_plan4, 1, inx, inembed, 1,  256, onembed, 1,  256, CUFFT_Z2Z, 131072);

	cufftHandle fft_plan5;
	inx[0] = inembed[0] = onembed[0] = 256;
	cufftPlanMany(&fft_plan5, 1, inx, inembed, 1,  256, onembed, 1,  256, CUFFT_Z2Z, 131072);
	...
	cudaMemcpy(B0, B0_host, 67108864 * sizeof(std::complex<double>), cudaMemcpyHostToDevice);	

	// create measuring parameters
	cudaEvent_t counters[23];
	for(int c = 0; c < 23; ++c)
	{
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 256, 256, 512, 33554432, B0, 33554432, temp0);
		// add measuring things
		cudaEventRecord(counters[1]);
		pack_data(16384, 4, 512, 33554432, (double*) temp0, 33554432, (double*) temp1);
		// add measuring things
		...
		unpack_data(64, 131072, 4, 33554432, (double*) temp0, 33554432, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[4]);
		fft_compute(CUFFT_FORWARD, fft_plan1, 1, 256, 131072, 33554432, temp1, 33554432, temp0);
		// add measuring things
		cudaEventRecord(counters[5]);
		pack_data(4, 64, 131072, 33554432, (double*) temp0, 33554432, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[6]);
		MPI_Alltoall((double*) temp1, 33554432/64, MPI_DOUBLE_COMPLEX, (double*) temp0, 33554432/64,  MPI_DOUBLE_COMPLEX, comm2);
		// add measuring things
		cudaEventRecord(counters[7]);
		unpack_data(16, 32768, 64, 33554432, (double*) temp0, 33554432, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[8]);
		fft_compute(CUFFT_FORWARD, fft_plan2, 4, 256, 32768, 33554432, temp1, 33554432, temp0);
		// add measuring things
		cudaEventRecord(counters[9]);
		fft_compute(CUFFT_INVERSE, fft_plan3, 4, 256, 32768, 33554432, temp0, 33554432, temp1);
		// add measuring things
		cudaEventRecord(counters[10]);
		pack_data(256, 4, 32768, 33554432, (double*) temp1, 33554432, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[11]);
		MPI_Alltoall((double*) temp0, 33554432/4, MPI_DOUBLE_COMPLEX, (double*) temp1, 33554432/4,  MPI_DOUBLE_COMPLEX, comm1);
		// add measuring things
		cudaEventRecord(counters[12]);
		unpack_data(16384, 512, 4, 33554432, (double*) temp1, 33554432, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[13]);
		fft_compute(CUFFT_INVERSE, fft_plan4, 256, 256, 512, 33554432, temp0, 33554432, temp1);
		// add measuring things
		cudaEventRecord(counters[14]);
		pack_data(1024, 64, 512, 33554432, (double*) temp1, 33554432, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[15]);
		MPI_Alltoall((double*) temp0, 33554432/64, MPI_DOUBLE_COMPLEX, (double*) temp1, 33554432/64,  MPI_DOUBLE_COMPLEX, comm2);
		// add measuring things
		cudaEventRecord(counters[16]);
		unpack_data(4, 131072, 64, 33554432, (double*) temp1, 33554432, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[17]);
		fft_compute(CUFFT_INVERSE, fft_plan5, 1, 256, 131072, 33554432, temp0, 33554432, temp1);
		// add measuring things
		cudaEventRecord(counters[18]);
		MPI_Allgather((double*) temp1, 33554432, MPI_DOUBLE_COMPLEX, (double*) temp0, 134217728,  MPI_DOUBLE_COMPLEX, comm1);
		// add measuring things
		cudaEventRecord(counters[19]);
		unpack_data(16384, 2048, 4, 134217728, (double*) temp0, 134217728, (double*) Tx0);
		// add measuring things
		cudaEventRecord(counters[20]);
		gemm_compute(gemm_plan, 256, 262144, 512, 67108864, A0, 134217728, Tx0, 131072, temp0);
		// add measuring things
	...
	if(rank == 0)
		printf("1024 16777216\tDI\t2 4 64\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
