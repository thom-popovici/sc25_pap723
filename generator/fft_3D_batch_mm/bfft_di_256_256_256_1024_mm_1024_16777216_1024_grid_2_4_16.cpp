...
void function(int cold_runs, int hot_runs)
{
	...
	B0_host = (std::complex<double>*) malloc(134217728 * sizeof(std::complex<double>));	
	...
	cufftPlanMany(&fft_plan0, 1, inx, inembed, 1,  256, onembed, 1,  256, CUFFT_Z2Z, 524288);

	cufftHandle fft_plan1;
	inx[0] = inembed[0] = onembed[0] = 256;
	cufftPlanMany(&fft_plan1, 1, inx, inembed, 1,  256, onembed, 1,  256, CUFFT_Z2Z, 524288);

	cufftHandle fft_plan2;
	inx[0] = inembed[0] = onembed[0] = 256;
	cufftPlanMany(&fft_plan2, 1, inx, inembed, 1,  256, onembed, 1,  256, CUFFT_Z2Z, 524288);

	cufftHandle fft_plan3;
	inx[0] = inembed[0] = onembed[0] = 256;
	cufftPlanMany(&fft_plan3, 1, inx, inembed, 1,  256, onembed, 1,  256, CUFFT_Z2Z, 524288);

	cufftHandle fft_plan4;
	inx[0] = inembed[0] = onembed[0] = 256;
	cufftPlanMany(&fft_plan4, 1, inx, inembed, 1,  256, onembed, 1,  256, CUFFT_Z2Z, 524288);

	cufftHandle fft_plan5;
	inx[0] = inembed[0] = onembed[0] = 256;
	cufftPlanMany(&fft_plan5, 1, inx, inembed, 1,  256, onembed, 1,  256, CUFFT_Z2Z, 524288);
	...
	cudaMalloc((void**)&B0, 134217728 * sizeof(std::complex<double>));	
	cudaMalloc((void**)&C0, 1048576 * sizeof(std::complex<double>));	
	cudaMalloc((void**)&temp, 536870912 * sizeof(std::complex<double>));	
	std::complex<double> *temp0 = (temp + 0 * 268435456);
	std::complex<double> *temp1 = (temp + 1 * 268435456);
	cudaMalloc((void**)&aux, 268435456 * sizeof(std::complex<double>));	
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 1, 256, 524288, 134217728, B0, 134217728, temp0);
		// add measuring things
		cudaEventRecord(counters[1]);
		pack_data(128, 2, 524288, 134217728, (double*) temp0, 134217728, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[2]);
		MPI_Alltoall((double*) temp1, 134217728/2, MPI_DOUBLE_COMPLEX, (double*) temp0, 134217728/2,  MPI_DOUBLE_COMPLEX, comm0);
		// add measuring things
		cudaEventRecord(counters[3]);
		unpack_data(16384, 4096, 2, 134217728, (double*) temp0, 134217728, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[4]);
		fft_compute(CUFFT_FORWARD, fft_plan1, 128, 256, 4096, 134217728, temp1, 134217728, temp0);
		// add measuring things
		cudaEventRecord(counters[5]);
		pack_data(2048, 16, 4096, 134217728, (double*) temp0, 134217728, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[6]);
		MPI_Alltoall((double*) temp1, 134217728/16, MPI_DOUBLE_COMPLEX, (double*) temp0, 134217728/16,  MPI_DOUBLE_COMPLEX, comm2);
		// add measuring things
		cudaEventRecord(counters[7]);
		unpack_data(32768, 256, 16, 134217728, (double*) temp0, 134217728, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[8]);
		fft_compute(CUFFT_FORWARD, fft_plan2, 2048, 256, 256, 134217728, temp1, 134217728, temp0);
		// add measuring things
		cudaEventRecord(counters[9]);
		fft_compute(CUFFT_INVERSE, fft_plan3, 2048, 256, 256, 134217728, temp0, 134217728, temp1);
		// add measuring things
		cudaEventRecord(counters[10]);
		pack_data(262144, 2, 256, 134217728, (double*) temp1, 134217728, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[11]);
		MPI_Alltoall((double*) temp0, 134217728/2, MPI_DOUBLE_COMPLEX, (double*) temp1, 134217728/2,  MPI_DOUBLE_COMPLEX, comm0);
		// add measuring things
		cudaEventRecord(counters[12]);
		unpack_data(128, 524288, 2, 134217728, (double*) temp1, 134217728, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[13]);
		fft_compute(CUFFT_INVERSE, fft_plan4, 1, 256, 524288, 134217728, temp0, 134217728, temp1);
		// add measuring things
		cudaEventRecord(counters[14]);
		pack_data(16, 16, 524288, 134217728, (double*) temp1, 134217728, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[15]);
		MPI_Alltoall((double*) temp0, 134217728/16, MPI_DOUBLE_COMPLEX, (double*) temp1, 134217728/16,  MPI_DOUBLE_COMPLEX, comm2);
		// add measuring things
		cudaEventRecord(counters[16]);
		unpack_data(256, 32768, 16, 134217728, (double*) temp1, 134217728, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[17]);
		fft_compute(CUFFT_INVERSE, fft_plan5, 16, 256, 32768, 134217728, temp0, 134217728, temp1);
		// add measuring things
		cudaEventRecord(counters[18]);
		MPI_Allgather((double*) temp1, 134217728, MPI_DOUBLE_COMPLEX, (double*) temp0, 268435456,  MPI_DOUBLE_COMPLEX, comm0);
		// add measuring things
		cudaEventRecord(counters[19]);
		unpack_data(524288, 256, 2, 268435456, (double*) temp0, 268435456, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[20]);
		pack_data(65536, 16, 256, 268435456, (double*) temp1, 268435456, (double*) temp0);
		// add measuring things
		...
		unpack_data(16, 1048576, 16, 268435456, (double*) temp1, 268435456, (double*) Tx0);
		// add measuring things
		cudaEventRecord(counters[23]);
		gemm_compute(gemm_plan, 512, 1048576, 256, 536870912, A0, 268435456, Tx0, 131072, temp0);
		// add measuring things
	...
	if(rank == 0)
		printf("1024 16777216\tDI\t2 4 16\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
