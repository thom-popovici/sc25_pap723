...
void function(int cold_runs, int hot_runs)
{
	...
	MPI_Comm_split(MPI_COMM_WORLD, rank % 2 + 2 * (rank / (2 * 2)), rank, &comm1);
	MPI_Comm_split(MPI_COMM_WORLD, rank % (2 * 2), rank, &comm2);

	// create arrays
	std::complex<double> *A0_host = NULL, *B0_host = NULL, *C0_host = NULL;
	A0_host = (std::complex<double>*) malloc(536870912 * sizeof(std::complex<double>));	
	B0_host = (std::complex<double>*) malloc(268435456 * sizeof(std::complex<double>));	
	...
	cufftPlanMany(&fft_plan0, 1, inx, inembed, 1,  256, onembed, 1,  256, CUFFT_Z2Z, 1048576);

	cufftHandle fft_plan1;
	inx[0] = inembed[0] = onembed[0] = 256;
	cufftPlanMany(&fft_plan1, 1, inx, inembed, 1,  256, onembed, 1,  256, CUFFT_Z2Z, 1048576);

	cufftHandle fft_plan2;
	inx[0] = inembed[0] = onembed[0] = 256;
	cufftPlanMany(&fft_plan2, 1, inx, inembed, 1,  256, onembed, 1,  256, CUFFT_Z2Z, 1048576);

	cufftHandle fft_plan3;
	inx[0] = inembed[0] = onembed[0] = 256;
	cufftPlanMany(&fft_plan3, 1, inx, inembed, 1,  256, onembed, 1,  256, CUFFT_Z2Z, 1048576);

	cufftHandle fft_plan4;
	inx[0] = inembed[0] = onembed[0] = 256;
	cufftPlanMany(&fft_plan4, 1, inx, inembed, 1,  256, onembed, 1,  256, CUFFT_Z2Z, 1048576);

	cufftHandle fft_plan5;
	inx[0] = inembed[0] = onembed[0] = 256;
	cufftPlanMany(&fft_plan5, 1, inx, inembed, 1,  256, onembed, 1,  256, CUFFT_Z2Z, 1048576);
	...
	cudaMalloc((void**)&A0, 536870912 * sizeof(std::complex<double>));	
	cudaMalloc((void**)&B0, 268435456 * sizeof(std::complex<double>));	
	cudaMalloc((void**)&C0, 1048576 * sizeof(std::complex<double>));	
	cudaMalloc((void**)&temp, 1073741824 * sizeof(std::complex<double>));	
	std::complex<double> *temp0 = (temp + 0 * 536870912);
	std::complex<double> *temp1 = (temp + 1 * 536870912);
	cudaMalloc((void**)&aux, 536870912 * sizeof(std::complex<double>));	
	std::complex<double>* Tx0 = temp;

	// copy data from arrays to device arrays
	cudaMemcpy(A0, A0_host, 536870912 * sizeof(std::complex<double>), cudaMemcpyHostToDevice);	
	cudaMemcpy(B0, B0_host, 536870912 * sizeof(std::complex<double>), cudaMemcpyHostToDevice);	
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 16, 256, 65536, 268435456, B0, 268435456, temp0);
		// add measuring things
		cudaEventRecord(counters[1]);
		pack_data(2048, 2, 65536, 268435456, (double*) temp0, 268435456, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[2]);
		MPI_Alltoall((double*) temp1, 268435456/2, MPI_DOUBLE_COMPLEX, (double*) temp0, 268435456/2,  MPI_DOUBLE_COMPLEX, comm0);
		// add measuring things
		cudaEventRecord(counters[3]);
		unpack_data(262144, 512, 2, 268435456, (double*) temp0, 268435456, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[4]);
		fft_compute(CUFFT_FORWARD, fft_plan1, 2048, 256, 512, 268435456, temp1, 268435456, temp0);
		// add measuring things
		cudaEventRecord(counters[5]);
		pack_data(32768, 16, 512, 268435456, (double*) temp0, 268435456, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[6]);
		MPI_Alltoall((double*) temp1, 268435456/16, MPI_DOUBLE_COMPLEX, (double*) temp0, 268435456/16,  MPI_DOUBLE_COMPLEX, comm2);
		// add measuring things
		cudaEventRecord(counters[7]);
		unpack_data(16, 1048576, 16, 268435456, (double*) temp0, 268435456, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[8]);
		fft_compute(CUFFT_FORWARD, fft_plan2, 1, 256, 1048576, 268435456, temp1, 268435456, temp0);
		// add measuring things
		cudaEventRecord(counters[9]);
		fft_compute(CUFFT_INVERSE, fft_plan3, 1, 256, 1048576, 268435456, temp0, 268435456, temp1);
		// add measuring things
		cudaEventRecord(counters[10]);
		pack_data(128, 2, 1048576, 268435456, (double*) temp1, 268435456, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[11]);
		MPI_Alltoall((double*) temp0, 268435456/2, MPI_DOUBLE_COMPLEX, (double*) temp1, 268435456/2,  MPI_DOUBLE_COMPLEX, comm0);
		// add measuring things
		cudaEventRecord(counters[12]);
		unpack_data(16384, 8192, 2, 268435456, (double*) temp1, 268435456, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[13]);
		fft_compute(CUFFT_INVERSE, fft_plan4, 128, 256, 8192, 268435456, temp0, 268435456, temp1);
		// add measuring things
		cudaEventRecord(counters[14]);
		pack_data(2048, 16, 8192, 268435456, (double*) temp1, 268435456, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[15]);
		MPI_Alltoall((double*) temp0, 268435456/16, MPI_DOUBLE_COMPLEX, (double*) temp1, 268435456/16,  MPI_DOUBLE_COMPLEX, comm2);
		// add measuring things
		cudaEventRecord(counters[16]);
		unpack_data(32768, 512, 16, 268435456, (double*) temp1, 268435456, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[17]);
		fft_compute(CUFFT_INVERSE, fft_plan5, 2048, 256, 512, 268435456, temp0, 268435456, temp1);
		// add measuring things
		cudaEventRecord(counters[18]);
		MPI_Allgather((double*) temp1, 268435456, MPI_DOUBLE_COMPLEX, (double*) temp0, 536870912,  MPI_DOUBLE_COMPLEX, comm0);
		// add measuring things
		cudaEventRecord(counters[19]);
		unpack_data(128, 2097152, 2, 536870912, (double*) temp0, 536870912, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[20]);
		pack_data(65536, 16, 512, 536870912, (double*) temp1, 536870912, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[21]);
		MPI_Alltoall((double*) temp0, 536870912/16, MPI_DOUBLE_COMPLEX, (double*) temp1, 536870912/16,  MPI_DOUBLE_COMPLEX, comm2);
		// add measuring things
		cudaEventRecord(counters[22]);
		unpack_data(4096, 8192, 16, 536870912, (double*) temp1, 536870912, (double*) Tx0);
		// add measuring things
		cudaEventRecord(counters[23]);
		gemm_compute(gemm_plan, 512, 1048576, 512, 536870912, A0, 536870912, Tx0, 262144, temp0);
		// add measuring things
		cudaEventRecord(counters[24]);
		MPI_Allreduce((double*) temp0, (double*) C0, 262144, MPI_DOUBLE_COMPLEX, MPI_SUM, comm2);
		// add measuring things
	...
	if(rank == 0)
		printf("1024 16777216\tDI\t2 2 16\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
