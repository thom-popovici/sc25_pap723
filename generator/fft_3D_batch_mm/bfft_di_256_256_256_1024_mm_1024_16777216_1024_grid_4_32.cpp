...
void function(int cold_runs, int hot_runs)
{
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan1, 2048, 256, 256, 134217728, temp0, 134217728, temp1);
		// add measuring things
		cudaEventRecord(counters[2]);
		pack_data(8, 32, 524288, 134217728, (double*) temp1, 134217728, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[3]);
		MPI_Alltoall((double*) temp0, 134217728/32, MPI_DOUBLE_COMPLEX, (double*) temp1, 134217728/32,  MPI_DOUBLE_COMPLEX, comm1);
		// add measuring things
		cudaEventRecord(counters[4]);
		unpack_data(64, 65536, 32, 134217728, (double*) temp1, 134217728, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[5]);
		fft_compute(CUFFT_FORWARD, fft_plan2, 8, 256, 65536, 134217728, temp0, 134217728, temp1);
		// add measuring things
		cudaEventRecord(counters[6]);
		fft_compute(CUFFT_INVERSE, fft_plan3, 8, 256, 65536, 134217728, temp1, 134217728, temp0);
		// add measuring things
		cudaEventRecord(counters[7]);
		fft_compute(CUFFT_INVERSE, fft_plan4, 2048, 256, 256, 134217728, temp0, 134217728, temp1);
		// add measuring things
		...
		unpack_data(8, 524288, 32, 134217728, (double*) temp1, 134217728, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[11]);
		fft_compute(CUFFT_INVERSE, fft_plan5, 1, 256, 524288, 134217728, temp0, 134217728, Tx0);
		// add measuring things
		cudaEventRecord(counters[12]);
		gemm_compute(gemm_plan, 1024, 524288, 256, 536870912, A0, 134217728, Tx0, 262144, temp0);
		// add measuring things
	...
	if(rank == 0)
		printf("1024 16777216\tDI\t4 32\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
