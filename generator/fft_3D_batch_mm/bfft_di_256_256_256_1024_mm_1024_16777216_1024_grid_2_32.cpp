...
void function(int cold_runs, int hot_runs)
{
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 8, 256, 131072, 268435456, B0, 268435456, temp0);
		// add measuring things
		cudaEventRecord(counters[1]);
		fft_compute(CUFFT_FORWARD, fft_plan1, 2048, 256, 512, 268435456, temp0, 268435456, temp1);
		// add measuring things
		cudaEventRecord(counters[2]);
		pack_data(16384, 32, 512, 268435456, (double*) temp1, 268435456, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[3]);
		MPI_Alltoall((double*) temp0, 268435456/32, MPI_DOUBLE_COMPLEX, (double*) temp1, 268435456/32,  MPI_DOUBLE_COMPLEX, comm1);
		// add measuring things
		cudaEventRecord(counters[4]);
		unpack_data(8, 1048576, 32, 268435456, (double*) temp1, 268435456, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[5]);
		fft_compute(CUFFT_FORWARD, fft_plan2, 1, 256, 1048576, 268435456, temp0, 268435456, temp1);
		// add measuring things
		cudaEventRecord(counters[6]);
		fft_compute(CUFFT_INVERSE, fft_plan3, 1, 256, 1048576, 268435456, temp1, 268435456, temp0);
		// add measuring things
		cudaEventRecord(counters[7]);
		fft_compute(CUFFT_INVERSE, fft_plan4, 256, 256, 4096, 268435456, temp0, 268435456, temp1);
		// add measuring things
		cudaEventRecord(counters[8]);
		pack_data(2048, 32, 4096, 268435456, (double*) temp1, 268435456, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[9]);
		MPI_Alltoall((double*) temp0, 268435456/32, MPI_DOUBLE_COMPLEX, (double*) temp1, 268435456/32,  MPI_DOUBLE_COMPLEX, comm1);
		// add measuring things
		cudaEventRecord(counters[10]);
		unpack_data(16384, 512, 32, 268435456, (double*) temp1, 268435456, (double*) temp0);
		// add measuring things
		...
		unpack_data(2048, 4096, 32, 268435456, (double*) temp1, 268435456, (double*) Tx0);
		// add measuring things
		cudaEventRecord(counters[15]);
		gemm_compute(gemm_plan, 1024, 524288, 512, 536870912, A0, 268435456, Tx0, 524288, temp0);
		// add measuring things
	...
	if(rank == 0)
		printf("1024 16777216\tDI\t2 32\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
