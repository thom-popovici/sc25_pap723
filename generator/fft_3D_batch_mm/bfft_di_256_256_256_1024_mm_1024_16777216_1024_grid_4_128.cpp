...
void function(int cold_runs, int hot_runs)
{
	...
	cudaMalloc((void**)&aux, 33554432 * sizeof(std::complex<double>));	
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 1, 256, 131072, 33554432, B0, 33554432, temp0);
		// add measuring things
		cudaEventRecord(counters[1]);
		fft_compute(CUFFT_FORWARD, fft_plan1, 512, 256, 256, 33554432, temp0, 33554432, temp1);
		// add measuring things
		cudaEventRecord(counters[2]);
		pack_data(2, 128, 131072, 33554432, (double*) temp1, 33554432, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[3]);
		MPI_Alltoall((double*) temp0, 33554432/128, MPI_DOUBLE_COMPLEX, (double*) temp1, 33554432/128,  MPI_DOUBLE_COMPLEX, comm1);
		// add measuring things
		cudaEventRecord(counters[4]);
		unpack_data(4, 65536, 128, 33554432, (double*) temp1, 33554432, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[5]);
		fft_compute(CUFFT_FORWARD, fft_plan2, 2, 256, 65536, 33554432, temp0, 33554432, temp1);
		// add measuring things
		cudaEventRecord(counters[6]);
		pack_data(1024, 128, 256, 33554432, (double*) temp1, 33554432, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[7]);
		MPI_Alltoall((double*) temp0, 33554432/128, MPI_DOUBLE_COMPLEX, (double*) temp1, 33554432/128,  MPI_DOUBLE_COMPLEX, comm1);
		// add measuring things
		cudaEventRecord(counters[8]);
		unpack_data(2, 131072, 128, 33554432, (double*) temp1, 33554432, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[9]);
		fft_compute(CUFFT_INVERSE, fft_plan3, 1, 256, 131072, 33554432, temp0, 33554432, temp1);
		// add measuring things
		cudaEventRecord(counters[10]);
		fft_compute(CUFFT_INVERSE, fft_plan4, 256, 256, 512, 33554432, temp1, 33554432, temp0);
		// add measuring things
		cudaEventRecord(counters[11]);
		pack_data(512, 128, 512, 33554432, (double*) temp0, 33554432, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[12]);
		MPI_Alltoall((double*) temp1, 33554432/128, MPI_DOUBLE_COMPLEX, (double*) temp0, 33554432/128,  MPI_DOUBLE_COMPLEX, comm1);
		// add measuring things
		cudaEventRecord(counters[13]);
		unpack_data(1024, 256, 128, 33554432, (double*) temp0, 33554432, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[14]);
		fft_compute(CUFFT_INVERSE, fft_plan5, 512, 256, 256, 33554432, temp1, 33554432, temp0);
		// add measuring things
		cudaEventRecord(counters[15]);
		pack_data(1024, 128, 256, 33554432, (double*) temp0, 33554432, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[16]);
		MPI_Alltoall((double*) temp1, 33554432/128, MPI_DOUBLE_COMPLEX, (double*) temp0, 33554432/128,  MPI_DOUBLE_COMPLEX, comm1);
		// add measuring things
		cudaEventRecord(counters[17]);
		unpack_data(512, 512, 128, 33554432, (double*) temp0, 33554432, (double*) Tx0);
		// add measuring things
		cudaEventRecord(counters[18]);
		gemm_compute(gemm_plan, 1024, 131072, 256, 134217728, A0, 33554432, Tx0, 262144, temp0);
		// add measuring things
		cudaEventRecord(counters[19]);
		MPI_Allreduce((double*) temp0, (double*) C0, 262144, MPI_DOUBLE_COMPLEX, MPI_SUM, comm1);
		// add measuring things
	...
	if(rank == 0)
		printf("1024 16777216\tDI\t4 128\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
