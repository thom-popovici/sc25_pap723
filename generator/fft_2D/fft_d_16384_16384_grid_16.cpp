...
void function(int cold_runs, int hot_runs)
{
	...
	cudaMemcpy(B0, B0_host, 16777216 * sizeof(std::complex<double>), cudaMemcpyHostToDevice);	
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 1024, 16384, 1, 16777216, A0, 16777216, temp0);
		// add measuring things
		...
		fft_compute(CUFFT_FORWARD, fft_plan1, 1, 16384, 1024, 16777216, temp0, 16777216, B0);
		// add measuring things
	...
	cudaMemcpy(B0_host, B0, 16777216 * sizeof(std::complex<double>), cudaMemcpyDeviceToHost);	
