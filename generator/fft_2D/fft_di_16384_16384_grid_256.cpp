...
void function(int cold_runs, int hot_runs)
{
	...
	cufftPlanMany(&fft_plan0, 1, inx, inembed, 1,  16384, onembed, 1,  16384, CUFFT_Z2Z, 64);

	cufftHandle fft_plan1;
	inx[0] = inembed[0] = onembed[0] = 16384;
	cufftPlanMany(&fft_plan1, 1, inx, inembed, 1,  16384, onembed, 1,  16384, CUFFT_Z2Z, 64);

	cufftHandle fft_plan2;
	inx[0] = inembed[0] = onembed[0] = 16384;
	cufftPlanMany(&fft_plan2, 1, inx, inembed, 1,  16384, onembed, 1,  16384, CUFFT_Z2Z, 64);

	cufftHandle fft_plan3;
	inx[0] = inembed[0] = onembed[0] = 16384;
	cufftPlanMany(&fft_plan3, 1, inx, inembed, 1,  16384, onembed, 1,  16384, CUFFT_Z2Z, 64);
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 1, 16384, 64, 1048576, A0, 1048576, temp0);
		// add measuring things
		cudaEventRecord(counters[1]);
		pack_data(64, 256, 64, 1048576, (double*) temp0, 1048576, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[2]);
		MPI_Alltoall((double*) temp1, 1048576/256, MPI_DOUBLE_COMPLEX, (double*) temp0, 1048576/256,  MPI_DOUBLE_COMPLEX, comm0);
		// add measuring things
		cudaEventRecord(counters[3]);
		fft_compute(CUFFT_FORWARD, fft_plan1, 64, 16384, 1, 1048576, temp0, 1048576, temp1);
		// add measuring things
		cudaEventRecord(counters[4]);
		fft_compute(CUFFT_INVERSE, fft_plan2, 64, 16384, 1, 1048576, temp1, 1048576, temp0);
		// add measuring things
		cudaEventRecord(counters[5]);
		MPI_Alltoall((double*) temp0, 1048576/256, MPI_DOUBLE_COMPLEX, (double*) temp1, 1048576/256,  MPI_DOUBLE_COMPLEX, comm0);
		// add measuring things
		cudaEventRecord(counters[6]);
		unpack_data(64, 64, 256, 1048576, (double*) temp1, 1048576, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[7]);
		fft_compute(CUFFT_INVERSE, fft_plan3, 1, 16384, 64, 1048576, temp0, 1048576, C0);
		// add measuring things
	...
	if(rank == 0)
		printf("16384 16384\tDI\t256\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
