...
void function(int cold_runs, int hot_runs)
{
	...
	cufftPlanMany(&fft_plan0, 1, inx, inembed, 1,  16384, onembed, 1,  16384, CUFFT_Z2Z, 512);

	cufftHandle fft_plan1;
	inx[0] = inembed[0] = onembed[0] = 16384;
	cufftPlanMany(&fft_plan1, 1, inx, inembed, 1,  16384, onembed, 1,  16384, CUFFT_Z2Z, 512);

	cufftHandle fft_plan2;
	inx[0] = inembed[0] = onembed[0] = 16384;
	cufftPlanMany(&fft_plan2, 1, inx, inembed, 1,  16384, onembed, 1,  16384, CUFFT_Z2Z, 512);

	cufftHandle fft_plan3;
	inx[0] = inembed[0] = onembed[0] = 16384;
	cufftPlanMany(&fft_plan3, 1, inx, inembed, 1,  16384, onembed, 1,  16384, CUFFT_Z2Z, 512);
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 1, 16384, 512, 8388608, A0, 8388608, temp0);
		// add measuring things
		cudaEventRecord(counters[1]);
		pack_data(512, 32, 512, 8388608, (double*) temp0, 8388608, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[2]);
		MPI_Alltoall((double*) temp1, 8388608/32, MPI_DOUBLE_COMPLEX, (double*) temp0, 8388608/32,  MPI_DOUBLE_COMPLEX, comm0);
		// add measuring things
		cudaEventRecord(counters[3]);
		fft_compute(CUFFT_FORWARD, fft_plan1, 512, 16384, 1, 8388608, temp0, 8388608, temp1);
		// add measuring things
		cudaEventRecord(counters[4]);
		fft_compute(CUFFT_INVERSE, fft_plan2, 512, 16384, 1, 8388608, temp1, 8388608, temp0);
		// add measuring things
		cudaEventRecord(counters[5]);
		MPI_Alltoall((double*) temp0, 8388608/32, MPI_DOUBLE_COMPLEX, (double*) temp1, 8388608/32,  MPI_DOUBLE_COMPLEX, comm0);
		// add measuring things
		cudaEventRecord(counters[6]);
		unpack_data(512, 512, 32, 8388608, (double*) temp1, 8388608, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[7]);
		fft_compute(CUFFT_INVERSE, fft_plan3, 1, 16384, 512, 8388608, temp0, 8388608, C0);
		// add measuring things
