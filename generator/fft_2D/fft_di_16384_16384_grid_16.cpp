...
void function(int cold_runs, int hot_runs)
{
	...
	cufftPlanMany(&fft_plan0, 1, inx, inembed, 1,  16384, onembed, 1,  16384, CUFFT_Z2Z, 1024);

	cufftHandle fft_plan1;
	inx[0] = inembed[0] = onembed[0] = 16384;
	cufftPlanMany(&fft_plan1, 1, inx, inembed, 1,  16384, onembed, 1,  16384, CUFFT_Z2Z, 1024);

	cufftHandle fft_plan2;
	inx[0] = inembed[0] = onembed[0] = 16384;
	cufftPlanMany(&fft_plan2, 1, inx, inembed, 1,  16384, onembed, 1,  16384, CUFFT_Z2Z, 1024);

	cufftHandle fft_plan3;
	inx[0] = inembed[0] = onembed[0] = 16384;
	cufftPlanMany(&fft_plan3, 1, inx, inembed, 1,  16384, onembed, 1,  16384, CUFFT_Z2Z, 1024);
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan0, 1, 16384, 1024, 16777216, A0, 16777216, temp0);
		// add measuring things
		cudaEventRecord(counters[1]);
		pack_data(1024, 16, 1024, 16777216, (double*) temp0, 16777216, (double*) temp1);
		// add measuring things
		cudaEventRecord(counters[2]);
		MPI_Alltoall((double*) temp1, 16777216/16, MPI_DOUBLE_COMPLEX, (double*) temp0, 16777216/16,  MPI_DOUBLE_COMPLEX, comm0);
		// add measuring things
		cudaEventRecord(counters[3]);
		fft_compute(CUFFT_FORWARD, fft_plan1, 1024, 16384, 1, 16777216, temp0, 16777216, temp1);
		// add measuring things
		cudaEventRecord(counters[4]);
		fft_compute(CUFFT_INVERSE, fft_plan2, 1024, 16384, 1, 16777216, temp1, 16777216, temp0);
		// add measuring things
		cudaEventRecord(counters[5]);
		MPI_Alltoall((double*) temp0, 16777216/16, MPI_DOUBLE_COMPLEX, (double*) temp1, 16777216/16,  MPI_DOUBLE_COMPLEX, comm0);
		// add measuring things
		cudaEventRecord(counters[6]);
		unpack_data(1024, 1024, 16, 16777216, (double*) temp1, 16777216, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[7]);
		fft_compute(CUFFT_INVERSE, fft_plan3, 1, 16384, 1024, 16777216, temp0, 16777216, C0);
		// add measuring things
