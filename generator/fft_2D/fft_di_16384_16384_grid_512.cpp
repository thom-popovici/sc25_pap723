...
void function(int cold_runs, int hot_runs)
{
	...
	inx[0] = inembed[0] = onembed[0] = 16384;
	cufftPlanMany(&fft_plan2, 1, inx, inembed, 1,  16384, onembed, 1,  16384, CUFFT_Z2Z, 32);

	cufftHandle fft_plan3;
	inx[0] = inembed[0] = onembed[0] = 16384;
	cufftPlanMany(&fft_plan3, 1, inx, inembed, 1,  16384, onembed, 1,  16384, CUFFT_Z2Z, 32);
	...
	for(int r = 0; r < cold_runs + hot_runs; ++r)
	{
		...
		fft_compute(CUFFT_FORWARD, fft_plan1, 32, 16384, 1, 524288, temp0, 524288, temp1);
		// add measuring things
		cudaEventRecord(counters[4]);
		fft_compute(CUFFT_INVERSE, fft_plan2, 32, 16384, 1, 524288, temp1, 524288, temp0);
		// add measuring things
		cudaEventRecord(counters[5]);
		MPI_Alltoall((double*) temp0, 524288/512, MPI_DOUBLE_COMPLEX, (double*) temp1, 524288/512,  MPI_DOUBLE_COMPLEX, comm0);
		// add measuring things
		cudaEventRecord(counters[6]);
		unpack_data(32, 32, 512, 524288, (double*) temp1, 524288, (double*) temp0);
		// add measuring things
		cudaEventRecord(counters[7]);
		fft_compute(CUFFT_INVERSE, fft_plan3, 1, 16384, 32, 524288, temp0, 524288, C0);
		// add measuring things
	...
	if(rank == 0)
		printf("16384 16384\tDI\t512\tComputation\t%lf\tPacking\t%lf\tCommunication\t%lf\tTotal\t%lf\n", t_comp / hot_runs, t_pack / hot_runs, t_comm / hot_runs, (t_comp + t_pack + t_comm) / hot_runs);
